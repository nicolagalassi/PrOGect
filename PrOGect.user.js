...
// @license         MIT
// @copyright       2019 Oz (OGLight, MIT); 2026 PrOGect contributors
...
// ==/UserScript==
'use strict';
// PrOGect is a fork of OGLight 5.3.3 by Oz (MIT). The MIT licence requires the original copyright
// notice to be retained, so the @author/@copyright attribution above stays in every published copy.
// Comments below that describe what "OGLight" does are kept deliberately: they document the base's
...
const oglRedirection = localStorage.getItem('syl-redirect');
if(oglRedirection?.indexOf('https') > -1)
{
    ...
    localStorage.setItem('syl-redirect', false);
...
.ogl_miniStats > div { background:var(--secondaryReversed); }
.ogl_miniStats .ogl_header { border-bottom:1px solid #2a3540; }

/* PrOGect mini-recap — compact, cohesive rows (impeccable: tight rhythm, hairline dividers, tabular
   values right-aligned, --syl tokens). metal/crystal/deut stacked with OGame's native flat .resourceIcon
   glyphs (zoom-shrunk, crop untouched); MO + artifacts on two columns; MSU as the emphasized summary row. */
.ogl_miniStats { width:150px; }
.ogl_miniStats .ogl_header { height:auto !important; padding:4px 7px !important; line-height:1.3 !important; }
.ogl_miniStats > .ogl_miniStatsBody { display:flex !important; flex-direction:column; gap:0 !important; grid-template-columns:none !important; padding:4px !important; }
/* three main resources: one per row, icon left / colored tabular value right, hairline divider between */
.ogl_miniStats .ogl_miniRes { display:flex !important; align-items:center; gap:2px; padding:2px 7px !important; }
.ogl_miniStats .ogl_miniRes + .ogl_miniRes { border-top:1px solid var(--syl-border, #26313d); }
.ogl_miniStats .ogl_miniRes .resourceIcon { flex:0 0 auto; zoom:.58; }
.ogl_miniStats .ogl_miniRes > span { margin-left:auto; font-size:12px; font-variant-numeric:tabular-nums; }
/* dark matter + artifacts: two tight columns, icon + value inline (override the base grid cell) */
.ogl_miniDual { display:grid; grid-template-columns:1fr 1fr; gap:4px; margin-top:5px; }
.ogl_miniStats .ogl_miniDual .ogl_icon { display:flex !important; align-items:center; gap:5px; grid-gap:0 !important; margin:0 !important; padding:3px 7px !important; }
.ogl_miniStats .ogl_miniDual .ogl_icon:before { margin:0 !important; zoom:.7; }
.ogl_miniStats .ogl_miniDual .ogl_icon > span { margin-left:auto; font-size:12px; font-variant-numeric:tabular-nums; }
/* MSU: emphasized summary row, its own surface */
.ogl_miniStats .ogl_miniMsu { margin-top:5px !important; padding:3px 8px !important; background:var(--secondaryReversed) !important; border-radius:var(--syl-radius, 4px); }
.ogl_miniStats .ogl_miniMsu > span { margin-left:auto; font-size:12px; font-weight:600; font-variant-numeric:tabular-nums; }

/* PrOGect stats popup — full restyle (impeccable): cohesive dark cards, a segmented range control,
   a violet-accented daily graph + active tab, and a readable recap table with an emphasized Total row.
   Tokens per DESIGN.md. The attacks tab drops the expedition-only pie + ship grid and goes single-column. */
.ogl_stats { display:flex !important; flex-direction:column; gap:14px; color:var(--syl-ink, #dfe7ee); font-size:13px; }

/* two-tab switcher (expeditions / attacks) */
.ogl_statsTabs { display:flex; gap:6px; margin:0; }
.ogl_statsTab { display:flex; align-items:center; gap:6px; padding:7px 16px; border-radius:var(--syl-radius, 4px); background:var(--secondary); color:var(--syl-muted, #8494a3); cursor:pointer; font-size:13px; font-weight:600; user-select:none; transition:background .15s ease-out, color .15s ease-out, box-shadow .15s ease-out; }
.ogl_statsTab .material-icons { font-size:17px; }
.ogl_statsTab:hover { color:var(--syl-ink, #dfe7ee); }
.ogl_statsTab.ogl_active { background:var(--syl-accent-weak, rgba(124,92,255,.16)); color:var(--syl-accent, #7c5cff); box-shadow:inset 0 0 0 1px var(--syl-accent, #7c5cff); }

/* range controls: a segmented card (D / W / M / All + month nav pushed right) */
.ogl_statsMonth { gap:6px; align-items:center; background:var(--syl-surface, #12181f); border:1px solid var(--syl-border, #26313d); border-radius:8px; padding:6px 8px; }
.ogl_statsMonth > * { padding:0 !important; }
.ogl_statsMonth .ogl_button { min-width:30px; height:26px; display:flex; align-items:center; justify-content:center; padding:0 11px !important; background:var(--secondary); border-radius:var(--syl-radius, 4px); color:var(--syl-muted, #8494a3); cursor:pointer; font-size:12px; font-weight:600; transition:color .15s ease-out; }
.ogl_statsMonth .ogl_button:hover { color:var(--syl-ink, #dfe7ee); }
.ogl_statsMonth .ogl_separator { flex:1; background:transparent !important; border:0 !important; }
.ogl_statsMonth > div:not(.ogl_button):not(.ogl_separator) { color:var(--syl-ink, #dfe7ee); font-weight:600; min-width:88px; text-align:center; }

/* daily graph card — diverging baseline: a zero line at the bar-zone centre, gains grow up (violet),
   losses grow down (red). Fixed 64px bar zone so the geometry is deterministic. */
.ogl_dateBar { position:relative; background:var(--syl-surface, #12181f) !important; border:1px solid var(--syl-border, #26313d); border-radius:10px !important; margin:0 !important; padding:10px 10px 24px !important; gap:2px; align-items:flex-start !important; }
.ogl_dateBar:before { content:''; position:absolute; left:10px; right:10px; top:42px; height:1px; background:var(--syl-border, #26313d); }
.ogl_dateBar .ogl_item { display:block !important; position:relative; height:64px; padding:0 !important; grid-template-rows:none !important; border-radius:6px; transition:background .12s ease-out; }
.ogl_dateBar > .ogl_item:hover { background:var(--p3, #1a222c); }
.ogl_dateBar .ogl_dbar { position:absolute; left:16%; width:68%; min-height:3px; pointer-events:none; }
.ogl_dateBar .ogl_dbar.ogl_pos { bottom:32px; border-radius:3px 3px 1px 1px; background:linear-gradient(to top, #5b3fd6, #8f74ff); }
.ogl_dateBar .ogl_dbar.ogl_neg { top:32px; border-radius:1px 1px 3px 3px; background:linear-gradient(to bottom, #c23b3b, #e05a5a); }
.ogl_dateBar .ogl_item:after { position:absolute; bottom:-20px; left:0; right:0; text-align:center; color:var(--syl-muted, #8494a3); opacity:.55; font-size:9px; pointer-events:none; }
.ogl_dateBar .ogl_item.ogl_today:after { color:var(--syl-accent, #7c5cff); opacity:1; font-weight:700; }
.ogl_dateBar .ogl_selected { background:var(--syl-accent-weak, rgba(124,92,255,.16)) !important; border-color:transparent !important; padding:0 !important; }

/* details grid: pie + ships on the expeditions tab, recap full width. The attacks tab hides the pie +
   ship grid but KEEPS the same 2-column width + a min-height so the popup doesn't resize on tab switch. */
.ogl_statsDetails { gap:14px !important; align-items:stretch !important; min-height:414px; align-content:start; }
.ogl_statsDetails h3 { grid-column:1/-1; margin:0 !important; padding:2px 2px 0; color:var(--syl-ink, #dfe7ee) !important; font-size:13px; font-weight:700; display:flex; align-items:center; justify-content:space-between; }
.ogl_statsRaidTab .ogl_pie, .ogl_statsRaidTab .ogl_shipTable { display:none !important; }
.ogl_statsRaidTab .ogl_sumTable { grid-column:1/-1; }

/* title toggles as small chips */
.ogl_statsSettingsIcons { display:flex; gap:6px; align-items:center; }
.ogl_statsSettingsIcons > * { display:flex; align-items:center; gap:3px; height:24px; padding:0 8px; background:var(--secondary); border:1px solid var(--syl-border, #26313d); border-radius:var(--syl-radius, 4px); color:var(--syl-muted, #8494a3); font-size:11px; }
.ogl_statsSettingsIcons > .ogl_active { color:var(--syl-accent, #7c5cff); border-color:var(--syl-accent, #7c5cff); }
.ogl_statsSettingsIcons .material-icons { font-size:15px; }

/* expedition outcome breakdown — ranked horizontal bars (replaces the donut), in the pie slot card */
.ogl_stats .ogl_pie { border:1px solid var(--syl-border, #26313d) !important; border-radius:10px !important; background:var(--syl-surface, #12181f) !important; flex-direction:column; align-items:stretch !important; justify-content:flex-start !important; gap:1px; height:auto !important; padding:12px 14px !important; }
.ogl_stats .ogl_pie:before { content:none !important; }
.ogl_outcomeHead { color:var(--syl-muted, #8494a3); font-size:11px; text-transform:uppercase; letter-spacing:.5px; margin-bottom:9px; }
.ogl_outcomeHead b { color:var(--syl-ink, #dfe7ee); font-size:14px; font-variant-numeric:tabular-nums; }
.ogl_outcomeRow { display:grid !important; grid-template-columns:10px 82px 1fr 32px 44px; align-items:center; gap:8px; height:23px; font-size:12px; }
.ogl_outcomeDot { width:9px; height:9px; border-radius:2px; }
.ogl_outcomeLabel { color:var(--syl-ink, #dfe7ee); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.ogl_outcomeTrack { height:7px; background:#0e141b; border-radius:4px; overflow:hidden; }
.ogl_outcomeFill { display:block; height:100%; border-radius:4px; transition:width .25s cubic-bezier(.2,.7,.3,1); }
.ogl_outcomeCount { text-align:right; font-variant-numeric:tabular-nums; font-weight:600; color:var(--syl-ink, #dfe7ee); }
.ogl_outcomePct { text-align:right; color:var(--syl-muted, #8494a3); font-variant-numeric:tabular-nums; }
@media (prefers-reduced-motion: reduce) { .ogl_outcomeFill { transition:none; } }

/* ACS per-report share chip on recycle reports (message list) */
.ogl_acsShare { display:inline-flex; align-items:center; gap:3px; margin-left:6px; padding:1px 6px; background:var(--secondary); border:1px solid var(--syl-border, #26313d); border-radius:var(--syl-radius, 4px); font-size:11px; color:var(--syl-muted, #8494a3); }
.ogl_acsShare .material-icons { font-size:13px; }
.ogl_acsShare input { width:26px; background:transparent !important; border:0 !important; color:var(--syl-ink, #dfe7ee) !important; text-align:right; font-size:11px; padding:0 !important; box-shadow:none !important; }
.ogl_acsShare input:focus { outline:none !important; background:transparent !important; color:var(--syl-ink, #dfe7ee) !important; box-shadow:none !important; }
.ogl_acsShareActive { border-color:var(--syl-accent, #7c5cff); color:var(--syl-accent, #7c5cff); }
.ogl_acsShareActive input { color:var(--syl-accent, #7c5cff); }

/* ship finds grid */
.ogl_stats .ogl_shipTable { gap:6px !important; align-content:start; }
.ogl_stats .ogl_shipTable > .ogl_icon { background:var(--syl-surface, #12181f) !important; border:1px solid var(--syl-border, #26313d) !important; border-radius:8px !important; padding:0 12px 0 0 !important; height:34px; color:var(--syl-ink, #dfe7ee); font-variant-numeric:tabular-nums; transition:border-color .15s ease-out; }
.ogl_stats .ogl_shipTable > .ogl_icon:hover { border-color:var(--syl-accent, #7c5cff); }
.ogl_stats .ogl_shipTable > .ogl_icon.ogl_off { opacity:.32; }

/* recap table */
.ogl_stats .ogl_sumTable { border:1px solid var(--syl-border, #26313d) !important; border-radius:10px !important; overflow:hidden; background:var(--syl-surface, #12181f) !important; margin-top:0 !important; }
.ogl_stats .ogl_sumTable > * { grid-template-columns:1.5fr .7fr repeat(6, 1fr) !important; line-height:30px !important; grid-gap:0 !important; }
.ogl_stats .ogl_sumTable > *:not(:last-child) { border-bottom:1px solid var(--syl-border, #26313d); }
.ogl_stats .ogl_sumTable > * > * { background:transparent !important; border-radius:0 !important; padding:0 6px; font-variant-numeric:tabular-nums; }
.ogl_stats .ogl_sumTable > *:first-child { background:#0e141b !important; }
.ogl_stats .ogl_sumTable .ogl_statsRecapHeader { text-align:left !important; padding-left:12px; color:var(--syl-muted, #8494a3); font-weight:600; text-transform:capitalize; }
.ogl_stats .ogl_sumTable > *:last-child { background:var(--syl-accent-weak, rgba(124,92,255,.12)) !important; font-weight:700; line-height:34px !important; }
.ogl_stats .ogl_sumTable > *:last-child .ogl_statsRecapHeader { color:var(--syl-accent, #7c5cff); }

@media (prefers-reduced-motion: reduce) { .ogl_statsTab, .ogl_dateBar .ogl_item, .ogl_stats .ogl_shipTable > .ogl_icon { transition:none; } }

/* PrOGect: settings redesign — two-pane tabbed modal (section nav + scrollable content pane +
   live search). Reuses OGLight tokens (--ogl accent, dark surfaces). Loaded after the main sheet
   so it wins on equal specificity. */
.ogl_config.ogl_configTabbed { width:660px !important; max-width:94vw; display:block !important; box-sizing:border-box; }
.ogl_configTabbed > h2 { margin-bottom:4px; }
.ogl_settingsSearch { box-sizing:border-box; width:100%; margin:2px 0 12px; padding:7px 12px; background:#121518; border:1px solid #080b10; border-radius:4px; color:#dfe6ee; font-size:13px; outline:none; }
.ogl_settingsSearch::placeholder { color:#6d8199; }
.ogl_settingsSearch:focus { border-color:var(--ogl); }
.ogl_settingsLayout { display:grid; grid-template-columns:172px minmax(0,1fr); gap:14px; align-items:start; }
.ogl_settingsNav { display:flex; flex-direction:column; gap:2px; }
.ogl_settingsNavItem { display:flex; align-items:center; gap:9px; width:100%; box-sizing:border-box; padding:8px 11px; background:none; border:0; border-radius:4px; color:#90aed5; font-size:13px; text-align:left; text-transform:capitalize; cursor:pointer; transition:background .15s ease-out, color .15s ease-out; }
.ogl_settingsNavItem > i { font-size:18px; }
.ogl_settingsNavItem:hover { background:#0e1116; color:#e7ecf3; }
.ogl_settingsNavItem.ogl_active { background:linear-gradient(-207deg,#0d1014,#212b34); color:var(--ogl); }
.ogl_settingsPane { display:grid; grid-gap:8px; align-content:start; min-width:0; height:60vh; overflow-y:auto; padding-right:8px; }
.ogl_settingsPane::-webkit-scrollbar { width:8px; }
.ogl_settingsPane::-webkit-scrollbar-thumb { background:#2a3540; border-radius:4px; }
.ogl_configTabbed [data-container] { display:none; max-height:none !important; padding:0; background:none; overflow:visible; }
.ogl_configTabbed [data-container].ogl_activePane { display:grid; grid-gap:8px; }
.ogl_configTabbed [data-container] > h3 { display:none; }
.ogl_configTabbed [data-container] > h3:after { content:none; }
.ogl_configTabbed.ogl_searching .ogl_settingsNav { opacity:.35; pointer-events:none; }
.ogl_configTabbed.ogl_searching [data-container] { display:grid !important; grid-gap:8px; }
.ogl_configTabbed.ogl_searching [data-container].ogl_hidden { display:none !important; }
.ogl_configTabbed.ogl_searching [data-container] > h3 { display:flex; pointer-events:none; margin-top:4px; }
@media (prefers-reduced-motion: reduce) { .ogl_settingsNavItem { transition:none; } }

/* PrOGect: settings inline detail view — sub-configs render here (with a Back button) instead of
   opening a second popup that would close the whole settings menu when dismissed. */
.ogl_settingsDetail { display:none; }
.ogl_configTabbed.ogl_showDetail .ogl_settingsLayout,
.ogl_configTabbed.ogl_showDetail .ogl_settingsSearch { display:none !important; }
.ogl_configTabbed.ogl_showDetail .ogl_settingsDetail { display:block; }
.ogl_settingsBack { width:auto !important; display:inline-flex !important; align-items:center; margin-bottom:12px; padding:5px 10px !important; }
.ogl_settingsDetailTitle { color:var(--syl-accent, var(--ogl)); text-align:left; margin:0 0 10px; text-transform:capitalize; }
.ogl_settingsDetailBody { max-height:54vh; overflow-y:auto; padding-right:6px; }
/* colored section icons in the settings nav (accents) */
.ogl_settingsNavItem[data-sec="fleet"] > i { color:#8b9bff; }
.ogl_settingsNavItem[data-sec="general"] > i { color:#9fb4c7; }
.ogl_settingsNavItem[data-sec="interface"] > i { color:#50c8ec; }
.ogl_settingsNavItem[data-sec="expeditions"] > i { color:#e4b633; }
.ogl_settingsNavItem[data-sec="stats"] > i { color:#43ec77; }
.ogl_settingsNavItem[data-sec="messages"] > i { color:#e0607a; }
.ogl_settingsNavItem[data-sec="PTRE"] > i { color:#b072ff; }
.ogl_settingsNavItem[data-sec="data"] > i { color:#38d7b2; }
/* detail view: give complex sub-configs room — widen the popup, center + scroll, one clean header */
.ogl_config.ogl_configTabbed.ogl_showDetail { width:840px !important; }
.ogl_settingsDetailBody { overflow:auto; max-height:58vh; padding:0 4px 6px; }
.ogl_settingsDetailBody > * { width:auto; margin:0 auto; }
.ogl_settingsDetailBody h2 { text-align:center; color:var(--syl-accent); margin:2px 0 14px; text-transform:capitalize; font-size:15px; }
.ogl_settingsDetailBody h2 .material-icons, .ogl_settingsDetailBody h2 i { margin-left:6px; }
/* PrOGect: from-scratch sub-config panels (limiter/keyboard/ships) — tabs, chips, clean rows */
.ogl_sylPanel { width:100%; max-width:540px; margin:0 auto; }
.ogl_sylPanel > h2 { text-align:center; color:var(--syl-accent); font-size:15px; margin:0 0 14px; display:flex; align-items:center; justify-content:center; gap:6px; }
.ogl_sylTabs { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; margin-bottom:12px; }
.ogl_sylTab { display:flex !important; align-items:center; justify-content:center; gap:6px; padding:9px 0 !important; text-transform:capitalize; }
.ogl_sylTab > i { font-size:17px; }
.ogl_sylTab.ogl_active { border-color:var(--syl-accent) !important; color:var(--syl-accent) !important; background:var(--syl-accent-weak) !important; }
.ogl_sylList { display:flex; flex-direction:column; gap:3px; }
.ogl_sylSync { display:flex; align-items:center; gap:8px; margin-bottom:8px; color:var(--syl-muted); font-size:12px; }
.ogl_sylChip { display:inline-flex !important; align-items:center; gap:5px; padding:5px 10px !important; text-transform:capitalize; }
.ogl_sylChip > i { font-size:15px; }
.ogl_sylRow { display:grid; grid-template-columns:28px 1fr 100px; align-items:center; gap:10px; padding:4px 8px; border-radius:6px; }
.ogl_sylRow:hover { background:#0e1116; }
.ogl_sylName { color:#c7cfd8; font-size:13px; }
.ogl_sylNum { text-align:right; background:#121518 !important; border:1px solid #080b10 !important; border-radius:4px !important; color:#dfe6ee !important; padding:5px 8px !important; width:100px !important; box-sizing:border-box !important; }
.ogl_sylNum:focus { border-color:var(--syl-accent) !important; outline:none; }
.ogl_sylKeyRow { grid-template-columns:1fr auto !important; }
.ogl_sylKeyChip { min-width:56px; text-align:center; text-transform:uppercase; font-weight:bold; padding:5px 12px !important; }
.ogl_sylKeyChip.ogl_active { border-color:var(--syl-accent) !important; color:var(--syl-accent) !important; }
.ogl_sylSaveBar { display:none; margin-top:12px; justify-content:center; }
.ogl_sylSaveBar.ogl_visible { display:flex; }
.ogl_sylShipGrid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; }
.ogl_sylShipCard { display:flex !important; flex-direction:column; align-items:center; gap:6px; padding:12px 6px !important; }
.ogl_sylShipCard > span { font-size:11px; color:#c7cfd8; }
.ogl_sylShipCard.ogl_active { border-color:var(--syl-accent) !important; background:var(--syl-accent-weak) !important; }
.ogl_sylShipCard.ogl_active > span { color:var(--syl-accent); }
/* PrOGect: real toggle-switch control (replaces OGLight checkboxes in settings) */
.ogl_sylToggle { position:relative; display:inline-block; width:38px; height:20px; border-radius:20px; background:#2a323c; flex:0 0 auto; transition:background .15s ease-out; }
.ogl_sylToggle:has(.ogl_sylToggleInput:checked) { background:var(--syl-accent); }
.ogl_sylToggleInput { position:absolute; inset:0; width:100%; height:100%; margin:0; opacity:0; cursor:pointer; }
.ogl_sylToggleKnob { position:absolute; top:2px; left:2px; width:16px; height:16px; border-radius:50%; background:#8a97a3; pointer-events:none; transition:transform .15s ease-out, background .15s ease-out; }
.ogl_sylToggleInput:checked ~ .ogl_sylToggleKnob { transform:translateX(18px); background:#fff; }
/* clean focus accent for the settings fields/selects */
.ogl_configTabbed label > input[type=text]:focus, .ogl_configTabbed label > input[type=password]:focus, .ogl_configTabbed label > select:focus { border-color:var(--syl-accent) !important; outline:none; }

/* ===== PrOGect SETTINGS — big page + internal panels (impeccable polish, product register) ===== */
.ogl_config.ogl_bigSettings { width:1000px !important; max-width:96vw !important; box-sizing:border-box; max-height:86vh; overflow-y:auto; overflow-x:hidden; padding:4px 8px 8px; }
.ogl_bigSettings > h2 { text-align:center; font-size:15px; font-weight:700; letter-spacing:.3px; color:var(--syl-ink); margin:2px 0 8px; padding-bottom:8px; border-bottom:1px solid var(--syl-border); }
/* the big-settings header spans full width and was painting over the popup close button at top-right,
   eating the click. The title is decorative, so let clicks pass through it; keep the close on top. */
.ogl_config.ogl_bigSettings > .ogl_close { z-index:1000010 !important; }
.ogl_bigSettings > h2 { pointer-events:none; }
.ogl_bigSettings > h2 .material-icons, .ogl_bigSettings > h2 i { margin-left:7px; color:var(--syl-accent); font-size:16px; }
.ogl_settingsGrid { columns:3; column-gap:12px; }
.ogl_bigSettings [data-container] { break-inside:avoid; -webkit-column-break-inside:avoid; display:block !important; max-height:none !important; overflow:visible !important; margin:0 0 10px !important; padding:0 !important; background:var(--syl-surface) !important; border:1px solid var(--syl-border); border-radius:8px; }
.ogl_bigSettings [data-container] > h3 { display:flex !important; align-items:center; gap:8px; margin:0 !important; padding:6px 12px !important; cursor:default !important; background:linear-gradient(to bottom, hsl(210deg 32% 13%), hsl(210deg 32% 10%)) !important; border-bottom:1px solid var(--syl-border); border-radius:8px 8px 0 0; color:var(--syl-ink); font-size:11px; font-weight:700; letter-spacing:.6px; text-transform:uppercase; }
.ogl_bigSettings [data-container] > h3:hover { box-shadow:none !important; color:var(--syl-ink) !important; }
.ogl_bigSettings [data-container] > h3:after { content:none !important; }
.ogl_bigSettings [data-container] > label { display:flex !important; align-items:center; gap:10px; min-height:26px; margin:0 !important; padding:3px 12px !important; border-top:1px solid hsl(210deg 32% 11%); color:var(--syl-muted); font-size:12px; transition:background .12s ease-out; }
.ogl_bigSettings [data-container] > h3 + label { border-top:0; }
.ogl_bigSettings [data-container] > label:hover { background:hsl(210deg 32% 11%); }
.ogl_bigSettings [data-container] > label:before { flex:1 1 auto; text-align:left; }
.ogl_bigSettings [data-container] > label > :first-child { margin-left:auto; }
.ogl_bigSettings [data-container] > label input[type=text], .ogl_bigSettings [data-container] > label input[type=password] { box-sizing:border-box; width:132px; padding:5px 9px; text-align:right; background:var(--syl-bg); border:1px solid var(--syl-border); border-radius:var(--syl-radius); color:var(--syl-ink); font-size:13px; outline:none; transition:border-color .12s ease-out; }
.ogl_bigSettings [data-container] > label input:focus { border-color:var(--syl-accent); }
.ogl_bigSettings [data-container] > label input::placeholder { color:var(--syl-muted); }
.ogl_bigSettings [data-container] > label select { box-sizing:border-box; width:132px; padding:5px 8px; background:var(--syl-bg); border:1px solid var(--syl-border); border-radius:var(--syl-radius); color:var(--syl-ink); font-size:13px; outline:none; cursor:pointer; }
.ogl_bigSettings [data-container] > label select:focus { border-color:var(--syl-accent); }
.ogl_bigSettings [data-container] > label > button { display:inline-flex; align-items:center; justify-content:center; width:auto; min-width:30px; height:28px; padding:0 9px; margin:0; background:var(--syl-bg); border:1px solid var(--syl-border); border-radius:var(--syl-radius); color:var(--syl-ink); font-size:16px; cursor:pointer; transition:border-color .12s ease-out, background .12s ease-out, color .12s ease-out; }
.ogl_bigSettings [data-container] > label > button:hover { border-color:var(--syl-accent); color:var(--syl-accent); background:var(--syl-accent-weak); }
.ogl_bigSettings [data-container] > label .ogl_icon { flex:0 0 auto; }
/* internal panel: keyboard (native .ogl_keyboardActions) */
.ogl_keyboardActions { display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:6px 22px; padding:2px; }
.ogl_keyboardActions > h2 { grid-column:1 / -1; text-align:center; font-size:15px; font-weight:700; color:var(--syl-ink); margin:2px 0 12px; padding-bottom:10px; border-bottom:1px solid var(--syl-border); }
.ogl_keyboardActions > button { grid-column:1 / -1; justify-self:center; margin-top:12px; padding:8px 26px; background:var(--syl-accent); border:0; border-radius:var(--syl-radius); color:#fff; font-weight:700; font-size:13px; cursor:pointer; }
.ogl_keyboardActions > button:hover { filter:brightness(1.08); }
.ogl_keyboardActions label { display:flex !important; align-items:center; justify-content:space-between; gap:12px; padding:5px 8px; border-radius:var(--syl-radius); color:var(--syl-muted); font-size:12.5px; }
.ogl_keyboardActions label:hover { background:hsl(210deg 32% 11%); }
.ogl_keyboardActions label input { box-sizing:border-box; width:50px; text-align:center; text-transform:uppercase; font-weight:700; padding:5px 4px; background:var(--syl-bg); border:1px solid var(--syl-border); border-radius:var(--syl-radius); color:var(--syl-ink); font-size:12.5px; outline:none; }
.ogl_keyboardActions label input:focus { border-color:var(--syl-accent); }
.ogl_keyboardActions label input.ogl_disabled, .ogl_keyboardActions label input:disabled { opacity:.45; cursor:not-allowed; }
/* internal panels: limiter / ships rhythm nudge */
.ogl_sylPanel > h2 { padding-bottom:10px; border-bottom:1px solid var(--syl-border); }
.ogl_sylRow { min-height:32px; }
/* internal panel: limiter — single CSS grid so every resource/ship row lines up label + 3 body inputs
   (was 4 independent flex columns that drifted). Fits + scrolls inside the settings detail body. */
.ogl_limiterSettings { max-width:600px; margin:0 auto; position:relative; }
.ogl_limiterSettings > h2 { text-align:center; font-size:15px; font-weight:700; color:var(--syl-ink); margin:2px 0 14px; padding-bottom:10px; border-bottom:1px solid var(--syl-border); }
.ogl_limiterSettings .ogl_limiterGrid { display:grid; grid-template-columns:34px repeat(3, minmax(0, 1fr)); gap:5px 10px; align-items:center; }
.ogl_limiterSettings .ogl_limiterHead { display:flex; flex-direction:column; align-items:center; gap:7px; padding-bottom:8px; }
.ogl_limiterSettings .ogl_limiterTitle { display:inline-flex; align-items:center; gap:5px; color:var(--syl-ink); font-size:12px; font-weight:700; text-transform:capitalize; }
.ogl_limiterSettings .ogl_limiterTitle i { color:var(--syl-accent); font-size:16px; }
.ogl_limiterSettings .ogl_limiterSync { display:flex; gap:4px; }
...
/* internal panel: spy table rules (ignore + highlight sets) */
.ogl_spyTableRules { display:flex; flex-direction:column; gap:16px; }
.ogl_spyRuleSet { display:flex; flex-direction:column; gap:8px; }
.ogl_spyTableRules .ogl_addRule { display:flex; flex-wrap:wrap; align-items:center; gap:6px; margin:0; padding:10px 12px; background:var(--syl-surface); border:1px solid var(--syl-border); border-radius:8px; }
.ogl_spyTableRules .ogl_addRule legend { padding:0 4px; display:flex; align-items:center; gap:6px; color:var(--syl-ink); font-weight:700; font-size:12.5px; }
.ogl_spyTableRules .ogl_addRule select, .ogl_spyTableRules .ogl_addRule input { box-sizing:border-box; padding:5px 8px !important; background:var(--syl-bg) !important; border:1px solid var(--syl-border) !important; border-radius:var(--syl-radius); color:var(--syl-ink) !important; font-size:12.5px; outline:none; }
.ogl_spyTableRules .ogl_addRule input { width:100px !important; text-align:right; }
.ogl_spyTableRules .ogl_addRule input:focus, .ogl_spyTableRules .ogl_addRule select:focus { border-color:var(--syl-accent) !important; }
.ogl_spyTableRules .ogl_addRule .ogl_button { display:inline-flex; align-items:center; gap:4px; padding:5px 12px; background:var(--syl-accent); border:0; border-radius:var(--syl-radius); color:#fff; font-size:12.5px; font-weight:700; cursor:pointer; }
...
.ogl_log > div:not(:first-child) { display:grid; grid-template-columns:150px 90px 1fr; gap:12px; padding:6px 10px; border-top:1px solid hsl(210deg 32% 11%); color:var(--syl-muted); font-size:12px; }
.ogl_log > div:not(:first-child) > div { min-width:0; overflow-wrap:anywhere; }
/* internal panel: manage data */
.ogl_manageData { position:relative; max-width:520px; margin:0 auto; }
.ogl_manageData > h2 { text-align:center; font-size:15px; font-weight:700; color:var(--syl-ink); margin:2px 0 14px; padding-bottom:10px; border-bottom:1px solid var(--syl-border); }
.ogl_manageData > .ogl_grid { display:flex !important; flex-direction:column; gap:8px; }
.ogl_manageData .ogl_button { box-sizing:border-box; display:flex !important; align-items:center; gap:10px; justify-content:flex-start; padding:9px 14px; background:var(--syl-surface); border:1px solid var(--syl-border); border-radius:8px; color:var(--syl-ink); font-size:13px; font-weight:600; cursor:pointer; text-decoration:none; transition:border-color .12s ease-out, background .12s ease-out; }
.ogl_manageData .ogl_button:hover { border-color:var(--syl-accent); background:var(--syl-accent-weak); }
.ogl_manageData .ogl_button .material-icons { font-size:18px; color:var(--syl-accent); }
.ogl_manageData .ogl_dangerZone { display:flex !important; flex-direction:column; gap:8px; margin:8px 0 0; padding:12px; background:rgba(224,115,107,.06); border:1px solid rgba(224,115,107,.35); border-radius:8px; }
.ogl_manageData .ogl_dangerZone legend { padding:0 6px; color:#e0736b; font-weight:700; font-size:12px; text-transform:uppercase; letter-spacing:.5px; }
.ogl_manageData .ogl_dangerZone .ogl_button { border-color:rgba(224,115,107,.4); }
.ogl_manageData .ogl_dangerZone .ogl_button:hover { border-color:#e0736b; background:rgba(224,115,107,.12); }
.ogl_manageData .ogl_dangerZone .ogl_button .material-icons { color:#e0736b; }
.ogl_manageData .ogl_hidden { display:none !important; }
/* PrOGect: merchant daily-offer — additive ONLY. A subtle rarity-colored ring on the Merchant
   button, shown only while today's offer is still claimable. Never hides/obscures/resizes/moves the
   button or swaps its image (AGENTS.md §1.7). Toggle: options.importExportItem. */
.ogl_ieHost { position:relative; }
.ogl_ieHost .menubutton { box-shadow:inset 0 0 0 2px var(--ie-col, transparent) !important; border-radius:5px; }
.ogl_ieHost[data-ie-rarity="common"] { --ie-col:#cd7f32; }   /* bronze */
.ogl_ieHost[data-ie-rarity="uncommon"] { --ie-col:#c9cdd2; } /* silver */
.ogl_ieHost[data-ie-rarity="rare"] { --ie-col:#ffcf40; }     /* gold */
.ogl_ieHost[data-ie-rarity="veryrare"], .ogl_ieHost[data-ie-rarity="very_rare"], .ogl_ieHost[data-ie-rarity="epic"], .ogl_ieHost[data-ie-rarity="legendary"] { --ie-col:#c77dff; } /* purple */
/* inline sub-config detail takes over the page */
.ogl_settingsDetail { display:none; }
.ogl_config.ogl_bigSettings.ogl_showDetail { width:860px !important; }
.ogl_bigSettings.ogl_showDetail > .ogl_settingsGrid, .ogl_bigSettings.ogl_showDetail > h2 { display:none !important; }
.ogl_config.ogl_showDetail .ogl_settingsDetail { display:block; }
@media (prefers-reduced-motion: reduce) { .ogl_bigSettings *, .ogl_keyboardActions * { transition:none !important; } }

#bar .OGameClock { padding-right:10px !important; }
.ogl_recap { border-top:2px solid #0e1116; }
.ogl_resourcesDetail h3 { margin-bottom:4px; text-align:center; }
.ogl_resourcesDetail h4 { border-bottom:2px solid #353c46; margin-bottom:6px; padding-bottom:6px; }
.ogl_planetTooltip { min-width:150px; }
.ogl_planetTooltip h3 { text-align:center; }
.ogl_planetTooltip img { border:1px solid #000; border-radius:50%; box-shadow:0 0 10px -3px #000; }
.ogl_notification .ogl_grid { max-width:275px; }
.ogl_universeInfoTooltip div { display:inline-block; }
.tippy-box .ogl_fleetDetail .ogl_button { text-align:left; }
.ogl_fleetDetail .ogl_icon:before { float:left; }
...
// CSS-only reskin of OGLight's OWN overlay (.ogl_* and the planet list it styles). No DOM changes;
// every element kept. Token-first: the palette lives once in :root and everything references it.
// OGame native chrome (left menu, resource bar, Merchant/Shop/Officers, ads, research/fleet pages)
// is deliberately NOT touched (AGENTS.md 1.7) — the tool only restyles itself. Loaded last so it wins.
GM_addStyle(`
:root {
    /* --- palette adopted from OGLight Legacy 4.2.7 (user reference, images 1-2): a cool blue-graphite
       ramp + a violet->teal accent gradient + cyan neon. Kept as the single source of truth; the
       --syl-* names below stay so the rest of the theme keeps referencing one place. --- */
    --p1:hsl(210deg 32% 6%);   /* base bg */
    --p2:hsl(210deg 32% 9%);   /* surface */
    --p3:hsl(210deg 32% 12%);  /* raised */
    --p4:hsl(210deg 32% 16%);  /* hairline border */
    --p5:hsl(210deg 32% 20%);  /* border strong / hover */
    --p6:hsl(210deg 32% 30%);
    ...
    --syl-ink:#dfe7ee;
    --syl-muted:#8494a3;
    --syl-accent:#7c5cff;                              /* violet end of the legacy gradient */
    --syl-accent-weak:hsl(255deg 90% 68% / .16);
    --syl-radius:4px;                                  /* legacy --uiradius */
    --syl-font:'Segoe UI',-apple-system,BlinkMacSystemFont,Roboto,Helvetica,Arial,sans-serif;
    /* --- remap OGLight's own tokens onto the palette (flattens every token-driven surface) --- */
    --ogl:var(--syl-accent);
    --highlight:var(--syl-accent);
    --primary:var(--p1);
    --secondary:var(--p2);
    --secondaryReversed:var(--p2);
    --tertiary:var(--p3);
    --date:hsl(224deg 100% 79%); --time:hsl(33deg 100% 66%); --texthighlight:hsl(199deg 64% 70%);
}

/* ---- typography: swap OGLight's Arial/Verdana for a clean system sans across the overlay ---- */
.ogl_side, .ogl_topbar, .ogl_popup, .ogl_popupContent, .tippy-box,
.ogl_recap, .ogl_miniStats, .smallplanet, .ogl_pinned, .ogl_tagged {
    font-family:var(--syl-font) !important;
    text-shadow:none !important;
}

/* ---- popups: one calm flat frame ---- */
.ogl_popupContent { background:var(--syl-surface) !important; border:1px solid var(--syl-border) !important; border-radius:8px !important; box-shadow:0 14px 44px -10px rgba(0,0,0,.7) !important; }
.ogl_popup { background:rgba(6,8,10,.55) !important; }
.ogl_popupContent ::-webkit-scrollbar { width:8px; height:8px; }
.ogl_popupContent ::-webkit-scrollbar-thumb { background:var(--syl-border-2); border-radius:4px; }
.ogl_popupContent ::-webkit-scrollbar-thumb:hover { background:#3a4150; }

/* ---- the signature glossy beveled OGLight button -> flat minimal surface (highest-impact) ---- */
.ogl_button, a.ogl_button {
    background:var(--syl-raised) !important;
    border:1px solid var(--syl-border-2) !important;
    border-radius:var(--syl-radius) !important;
    text-shadow:none !important;
    box-shadow:none !important;
    color:var(--syl-ink) !important;
    transition:background .15s ease-out, border-color .15s ease-out !important;
}
.ogl_button:hover, a.ogl_button:hover { background:#232b33 !important; border-color:#3a4450 !important; }
.ogl_button.ogl_active, a.ogl_button.ogl_active { background:var(--syl-accent-weak) !important; border-color:var(--syl-accent) !important; color:var(--syl-accent) !important; }

/* ---- topbar icon strip ---- */
.ogl_topbar { background:var(--syl-surface) !important; border:1px solid var(--syl-border) !important; border-radius:var(--syl-radius) !important; }

/* ---- side panel ---- */
.ogl_side { border-left:1px solid var(--syl-border) !important; }
.ogl_side.ogl_active { box-shadow:-8px 0 40px rgba(0,0,0,.5) !important; }
.ogl_side hr { background:var(--syl-border) !important; height:1px !important; }

/* ---- planet list: re-laid out as distinct rounded cards (OGame-Infinity feel), matte thumbnails,
   and a strong full-card violet selection so a selected planet OR moon is unmistakable ---- */
#planetList { grid-gap:5px !important; background:transparent !important; padding:4px 2px !important; }
.smallplanet { box-sizing:border-box !important; background:var(--syl-raised) !important; border:1px solid var(--syl-border) !important; border-radius:8px !important; box-shadow:none !important; overflow:visible !important; }
...
.tippy-box .tippy-arrow { color:var(--syl-surface) !important; }

/* ---- unify residual OGLight radii on its own surfaces ---- */
.ogl_miniStats, .ogl_recap, .ogl_list>div>*, .ogl_sideFleetIcon>:not(img), .ogl_pinned .ogl_tabs .ogl_active { border-radius:var(--syl-radius) !important; }

/* ---- topbar: legacy-style colored icon buttons (each action its own hue on the flat strip) ---- */
.ogl_topbar .ogl_tbHarvest  { color:#50deec !important; }  /* cyan  — collect resources */
.ogl_topbar .ogl_tbEmpire   { color:#b072ff !important; }  /* violet — account summary */
.ogl_topbar .ogl_tbTagged   { color:#e4b633 !important; }  /* gold  — tagged planets */
.ogl_topbar .ogl_tbPinned   { color:#43ec77 !important; }  /* green — pinned players */
.ogl_topbar .ogl_tbSettings { color:#9fb4c7 !important; }  /* steel — settings */
.ogl_topbar .ogl_tbSync     { color:#38d7b2 !important; }  /* teal  — sync empire */
.ogl_topbar .ogl_tbFleetSave { color:#8b9bff !important; }  /* indigo — night fleet save */
.ogl_topbar .ogl_tbSiblingShips { color:#1dd1a1 !important; }  /* mint — sibling ships */
.ogl_settingsFooter { display:flex; justify-content:center; margin-top:14px; padding-top:12px; border-top:1px solid #ffffff14; }
.ogl_settingsFooter .ogl_donate { color:#e0607a !important; display:inline-flex; align-items:center; gap:6px; }
.ogl_settingsFooter .ogl_donate .material-icons { color:#e0607a !important; font-size:18px; }
.ogl_topbar > i, .ogl_topbar > a { transition:filter .15s ease-out, transform .15s ease-out; }
.ogl_topbar > i:hover, .ogl_topbar > a:hover { filter:brightness(1.25); transform:translateY(-1px); }
`);
...
class PrOGect
{
    constructor(params)
    {
        ...
        const dbDefaults =
        {
            ...
            options:
            {
                ...
                collectLinked: false,
                cargoFallback: 0,   // PrOGect: 0=off 1=+other cargo 2=+pathfinder 3=+recycler
                expeditionValue: 0,
                ...
                importExportItem: true,
                reduceLargeImages: false,
                // '' = off, otherwise the palette name; see the body[data-colorblind] blocks in CSS
                colorblindMode: '',
                showMenuResources: 0,
                ...
                // Destination blank = current planet's coords. Mission/speed/leave-resources preset here.
                fsGalaxy: '',
                fsSystem: '',
                fsPosition: '',
                fsMission: 4,        // 4=deployment · options: 4 schieramento / 6 spionaggio / 7 colonizzazione / 8 riciclaggio
                fsSpeed: 10,         // FS speed as a PERCENT (10..100); mapped to the real speed-bar step at send time
                fsLeaveMetal: 0,
                fsLeaveCrystal: 0,
                fsLeaveDeut: 0,
                expeditionHoldTime: 8,   // hours held on expedition (Feature 2: night expeditions)
                keyboardActions:
                {
                    menu: '²',
                    previousPlanet: 'i',
                    ...
                    discovery: 'u',
                    showMenuResources: 'v',
                    fleetSave: 'n',          // PrOGect: night fleet save preset
                    expeditionNight: 'e'     // PrOGect: expedition with preset hold time (8h)
                }
            }
        }
        ...
        // PrOGect: keep a reachable copy of the original OGLight key map so the Keyboard
        // settings panel can offer a "reset to defaults" that restores the exact vanilla bindings.
        this.defaultKeyboardActions = Object.assign({}, dbDefaults.options.keyboardActions);
        ...
        // colorblindMode used to be a boolean; it is a palette name now. An existing `true` meant the
        // single old palette, which was tuned for red-green deficiency - deuteranopia is the closest
        // named equivalent, so nobody loses their setting on upgrade.
        if(typeof this.db.options.colorblindMode === 'boolean') this.db.options.colorblindMode = this.db.options.colorblindMode ? 'deuteranopia' : '';
        ...
        const checkDocumentState = () =>
        {
            if(document.readyState !== 'loading' && !this.isReady)
            {
                ...
                {
                    this.isReady = true;
                    // v13 compat: wrap init in try/catch so partial crashes don't silently kill all features
                    try
                    {
                    ...
                    {
                        console.error('[PrOGect v13] init() crashed:', e);
                    }
                }
                else
                {
    ...
    init(cache)
    {
        document.body.classList.add('progect');

        this.id = GM_getValue('ogl_id') || false;
        this.version = GM_info.script.version.indexOf('b') > -1 ? pgVersion+betaVersion : pgVersion;
        this.tooltipEvent = new Event('tooltip');

        this.mode = parseInt(new URLSearchParams(window.location.search).get('oglmode')) || 0; // 1:collect, 2:collectLinked, 3:todolist, 4:reportQuickRaid, 5:expedition, 6:nightFleetSave, 7:siblingShips
        ...
        this.ptreKey = localStorage.getItem('syl-ptreTK') || false;
        this.planetType = document.querySelector('head meta[name="ogame-planet-type"]')?.getAttribute('content') || 'planet';
        this.flagsList = ['friend', 'rush', 'danger', 'skull', 'fridge', 'star', 'trade', 'money', 'ptre', 'none'];

        this.server = {};
        this.server.id = window.location.host.replace(/\D/g,'');
        this.server.name = document.querySelector('head meta[name="ogame-universe-name"]')?.getAttribute('content') || window.location.host.split('.')[0] || 'unknown';
        this.server.lang = document.querySelector('head meta[name="ogame-language"]')?.getAttribute('content') || window.location.host.split(/[-.]/)[1] || 'en';
        this.server.economySpeed = parseInt(document.querySelector('head meta[name="ogame-universe-speed"]')?.getAttribute('content') || '1') || 1;
        const _fleetSpeedBase = parseInt(document.querySelector('head meta[name="ogame-universe-speed-fleet"]')?.getAttribute('content') || document.querySelector('head meta[name="ogame-universe-speed"]')?.getAttribute('content') || '1') || 1;
        this.server.peacefulFleetSpeed = parseInt(document.querySelector('head meta[name="ogame-universe-speed-fleet-peaceful"]')?.getAttribute('content') || _fleetSpeedBase);
        this.server.holdingFleetSpeed = parseInt(document.querySelector('head meta[name="ogame-universe-speed-fleet-holding"]')?.getAttribute('content') || _fleetSpeedBase);
        this.server.warFleetSpeed = parseInt(document.querySelector('head meta[name="ogame-universe-speed-fleet-war"]')?.getAttribute('content') || _fleetSpeedBase);
        // OGame's data endpoints differ by major version: the accountInfo export API is v13-only, so the
        // empire update needs the pre-v13 ajax endpoint as a fallback (see fetchAccountInfo/fetchAccountV12).
        // Defaults to 13 when the meta is missing: the v13 path has its own capability probe anyway.
        this.server.gameVersion = parseInt(document.querySelector('head meta[name="ogame-version"]')?.getAttribute('content')?.split('.')[0] || '13') || 13;
        ...
        const _charSprite = document.querySelector('#characterclass .sprite') || document.querySelector('[id*="characterclass"] .sprite') || document.querySelector('[class*="characterClass"] .sprite');
        this.account.class = _charSprite?.classList.contains('miner') ? 1 : _charSprite?.classList.contains('warrior') ? 2 : 3;
        this.account.name = document.querySelector('head meta[name="ogame-player-name"]')?.getAttribute('content') || '';
        this.account.rank = (document.querySelector('#bar a[href*="searchRelId"]') || document.querySelector('[id*="bar"] a[href*="searchRelId"]') || document.querySelector('a[href*="searchRelId"]'))?.parentNode?.innerText.replace(/\D/g, '') || '';
        const _langMatch = /oglocale=([a-z]+)/.exec(document.cookie);
        this.account.lang = _langMatch ? _langMatch[1] : (document.querySelector('head meta[name="ogame-language"]')?.getAttribute('content') || 'en');
        this.account.currentPlanetID = document.querySelector('head meta[name="ogame-planet-id"]')?.getAttribute('content') || '';
        this.account.chatEnabled = document.querySelector('#chatBar') || document.querySelector('[id*="chatBar"]') || document.querySelector('[id*="chat-bar"]');
        this.account.hasGeologist = document.querySelector('#officers .geologist.on') || document.querySelector('[id*="officers"] .geologist.on');
        ...
        if(this.page == 'overview')
        {
            ...
            document.querySelectorAll('#buffBar [data-uuid], [id*="buffBar"] [data-uuid], [class*="buffBar"] [data-uuid]').forEach(item =>
            {
        ...
        if(this.account.lang != this.db.userLang && this.db.userLang && this.page != 'fleetdispatch' && this.page != 'intro' && window.location.href.indexOf('component=fleetdispatch') < 0)
        {
        ...
        // v13 fix: ensure the destination picker + harvest/ship-harvest flags are removed on fresh page
        // load (prevents "any click redirects to fleet" bug from stale body class)
        if(this.page !== 'fleetdispatch')
        {
            document.body.classList.remove('ogl_destinationPicker');
            document.body.classList.remove('ogl_initHarvest');
            document.body.classList.remove('ogl_initShipHarvest');
        }
        ...
        if(this.page != 'empire')
        {
            ...
            // v13 compat: each Manager wrapped in its own try/catch so one failure doesn't cascade and kill all the others.
            // Errors are logged to window._ogl_init_errors (readable by the diagnostic) AND console.error.
            window._ogl_init_errors = window._ogl_init_errors || [];
            const _safeNew = (name, ctor) =>
            {
                try
                {
                    new ctor(this);
                ...
                {
                    const msg = '[PrOGect v13] ' + name + ' failed: ' + (e?.message || e) + '\n' + (e?.stack || '').split('\n').slice(0, 6).join('\n');
                    window._ogl_init_errors.push(msg);
                    console.error(msg);
                }
            };

            _safeNew('DomManager', DomManager);
            _safeNew('LangManager', LangManager);
            _safeNew('TimeManager', TimeManager);
            _safeNew('FetchManager', FetchManager);

            try { this.getPlanetData(); } catch(e) { window._ogl_init_errors.push('getPlanetData: ' + e.message); console.error('[PrOGect v13] getPlanetData failed:', e); }
            try { this.getServerData(); } catch(e) { window._ogl_init_errors.push('getServerData: ' + e.message); console.error('[PrOGect v13] getServerData failed:', e); }
            try { this.PTRE = new PTRE(this); } catch(e) { window._ogl_init_errors.push('PTRE: ' + e.message); console.error('[PrOGect v13] PTRE failed:', e); }

            _safeNew('UIManager', UIManager);
            _safeNew('ShortcutManager', ShortcutManager);
            _safeNew('TopbarManager', TopbarManager);
            _safeNew('FleetManager', FleetManager);
            _safeNew('AccountManager', AccountManager);
            _safeNew('TooltipManager', TooltipManager);
            _safeNew('NotificationManager', NotificationManager);
            _safeNew('StatsManager', StatsManager);
            _safeNew('GalaxyManager', GalaxyManager);
            _safeNew('MessageManager', MessageManager);
            _safeNew('HighscoreManager', HighscoreManager);
            _safeNew('MovementManager', MovementManager);
            _safeNew('TechManager', TechManager);
            _safeNew('JumpgateManager', JumpgateManager);
            _safeNew('EmpireManager', EmpireManager);
            ...
            const _logoEl = document.querySelector('#pageReloader') || document.querySelector('#logoLink') || document.querySelector('[id*="pageReloader"]') || document.querySelector('[id*="logoLink"]') || document.querySelector('[class*="logo"]');
            if(_logoEl)
            {
                _logoEl.classList.add('tooltipBottom');
                _logoEl.setAttribute('title', universeInfo);
            }
            const _pageContent = document.querySelector('#pageContent') || document.querySelector('[id*="pageContent"]') || document.querySelector('[id*="page-content"]') || document.querySelector('#contentholder') || document.body;
            _pageContent.appendChild(Util.addDom('div', { class:'ogl_universeName', child:`${this.server.name}.${this.server.lang}` }));
    ...
    getPlanetData(reloaded)
    {
        if(!reloaded)
        {
            ...
            this.currentPlanet.obj.id = this.account.currentPlanetID;
            this.currentPlanet.obj.type = document.querySelector('head meta[name="ogame-planet-type"]')?.getAttribute('content') || 'planet';
            ...
            {
                // update prod / sec
                let prod = resourcesBar.resources[resource.replace('deut', 'deuterium')]?.production || 0;
    ...
    updateJQuerySettings()
    {
        ...
        {
            // v13 compat: tab ID may be in URL param or POST data; match both page=messages and component=messages
            const _url = settings.url || '';
            const _data = settings.data || '';
            const _isMessages = _url.indexOf('page=messages') >= 0 || _url.indexOf('component=messages') >= 0;
            if(_isMessages && (_url.indexOf('tab=') >= 0 || _data.indexOf('tab=') >= 0 || _url.indexOf('activeSubTab=') >= 0 || _data.indexOf('activeSubTab=') >= 0))
            {
                const tabID = new URLSearchParams(_url).get('tab')
                    || new URLSearchParams(_data).get('tab')
                    || new URLSearchParams(_url).get('activeSubTab')
                    || new URLSearchParams(_data).get('activeSubTab');
        ...
        {
            const _settData = settings.data || '';
            if((settings.url.indexOf('action=getMessagesList') >= 0 || settings.url.indexOf('action=getMessages') >= 0) && _settData.indexOf('showTrash=true') < 0)
            {
                const tabID = parseInt(new URLSearchParams(_settData).get('activeSubTab'))
                    || parseInt(new URLSearchParams(settings.url).get('activeSubTab'))
                    || parseInt(new URLSearchParams(_settData).get('tab'))
                    || parseInt(new URLSearchParams(settings.url).get('tab'))
                    || -1;
                self._message.tabID = tabID;
                self._message.checkTab(); // check messages after pagination
            }
            else if((settings.url.indexOf('page=standalone') >= 0 && settings.url.indexOf('component=planetbar') >= 0) || settings.url.indexOf('component=planetbar') >= 0)
            {
            ...
            else if(settings.url.indexOf('action=flagDeleted') >= 0)
            {
                ...
                // v13 compat: action renamed to fetchSolarSystemData with asJson=1
                || settings.url.indexOf('action=fetchSolarSystemData') >= 0
                || settings.url.indexOf('action=fetchGalaxy') >= 0
                || (settings.url.indexOf('component=galaxy') >= 0 && settings.url.indexOf('ajax=1') >= 0)
                || (settings.url.indexOf('component=galaxy') >= 0 && settings.url.indexOf('asJson=1') >= 0)
            ) // check galaxy on system change
            {
                try
                {
                    const _gData = xhr.responseJSON || JSON.parse(xhr.responseText);
                    self._galaxy.check(_gData);
                }
                catch(e)
                {
                    console.error('PrOGect: failed to parse galaxy response', e);
                }
            }
            ...
            else if(settings.url.indexOf('component=eventList') >= 0 && (settings.url.indexOf('asJson=1') < 0 || settings.url.indexOf('fetchEventBox') >= 0))
            {
                const checkMovement = () =>
                {
                    setTimeout(() => // we wait to be sure xml as been added to the DOM and OGL has been red
                    {
                        ...
                        {
                            let _xmlToParse = xhr.responseText;
                            // New OGame may return JSON with HTML inside
                            try {
                                const _jsonResp = JSON.parse(xhr.responseText);
                                if(_jsonResp?.content?.eventlist) _xmlToParse = _jsonResp.content.eventlist; // v13: content.eventlist
                                else if(_jsonResp?.components?.eventList) _xmlToParse = _jsonResp.components.eventList; // older OGame
                                else if(typeof _jsonResp === 'string') _xmlToParse = _jsonResp;
                            } catch(e) {}
                            const xml = new DOMParser().parseFromString(_xmlToParse, 'text/html');
            ...
            else if(settings.url.indexOf('&component=movement') >= 0)
            {
                // v13 fix: recalling a fleet (action=recallFleet) and OGame's own movement-page
                // auto-refresh (page=standalone&component=movement, driven by timerHandler) both
                // replace #movementcomponent with fresh RAW markup. Nothing re-applied PrOGect's
                // styling, so the list rendered unstyled (bare coords/times). Re-run updateMovement()
                // once the fresh content is in the DOM, but only if a .fleetDetails is not yet styled
                // (updateMovement moves nodes, so re-processing an already-styled list would break it).
                setTimeout(() =>
                {
                    const _mc = document.querySelector('#movementcomponent');
                    const _hasRaw = _mc && Array.from(_mc.querySelectorAll('.fleetDetails')).some(f => !f.querySelector('.ogl_timeBlock'));
                    if(self._movement && _hasRaw) self._movement.updateMovement();
                }, 50);
            }
            ...
            {
                // v13 compat: #technologydetails may be renamed
        ...
        $.ajaxSetup(
        {
            beforeSend:function(xhr, settings)
            {
                if(this.url.indexOf('action=checkTarget') >= 0 && !self?._fleet?.firstLoadCancelled) // clear default fleet loading
                {
                    ...
                    // v13 compat: galaxy content action renamed to fetchSolarSystemData with asJson=1
                    this.url.indexOf('action=fetchGalaxyContent') >= 0
                    || this.url.indexOf('action=fetchSolarSystemData') >= 0
                    || this.url.indexOf('action=fetchGalaxy') >= 0
                    || (this.url.indexOf('component=galaxy') >= 0 && this.url.indexOf('ajax=1') >= 0)
                    || (this.url.indexOf('component=galaxy') >= 0 && this.url.indexOf('asJson=1') >= 0)
                ) // abort galaxy spam
                {
                    ...
                    // v13 compat: getMessagesList action may be renamed
                    this.url.indexOf('action=getMessagesList') >= 0
                    || this.url.indexOf('action=getMessages') >= 0
                ) // abort message spam
                {
                    if(self._message?.xhr) self._message.xhr.abort();
                    if(self._message) self._message.xhr = xhr;

                    const _settingsData = settings.data || '';
                    if(
                        (_settingsData.indexOf('activeSubTab=20') > -1 || _settingsData.indexOf('tab=20') > -1)
                        && (_settingsData.indexOf('showTrash=false') > -1 || _settingsData.indexOf('showTrash') < 0)
                    )
                    {
                        if(self._message) self._message.loadSpyTable(true);
                    }
                }
            }
        });
    ...
    calcExpeditionMax()
    {
        ...
        // New OGame uses 'ResultBooster' for expedition resource bonus (was 'ResourcesExpedition')
        // Characterclasses3 (explorer class bonus) is now included in ResultBooster total
        const _lf3bonus = this.db.lfBonuses?.Characterclasses3?.bonus || 0;
        const _lfExpBonus = this.db.lfBonuses?.ResourcesExpedition?.bonus
            || this.db.lfBonuses?.ResultBooster?.bonus
            || 0;
    ...
    loadPlanetList()
    {
        ...
        // v13 compat: helper to safely extract cp= id from a link element
        const _cpId = link =>
        {
            if(!link) return -1;
            const href = link.getAttribute('href') || '';
            let id = new URLSearchParams(href.split('?')[1] || '').get('cp')?.split('#')?.[0];
            if(!id) id = link.getAttribute('data-planetid') || link.getAttribute('data-moonid');
            return id || -1;
        };
        // v13 compat: helper to get coords from a smallplanet row
        const _koords = line =>
        {
            const k = line.querySelector('.planet-koords') || line.querySelector('[class*="koords"]') || line.querySelector('[class*="coords"]');
            if(!k) return '';
            return k.textContent.replace(/[\[\]]/g, '').trim();
        };

        document.querySelectorAll('.smallplanet').forEach(line =>
        {
            const planet = {};
            const planetLink = line.querySelector('.planetlink') || line.querySelector('a[href*="cp="]');
            // v13 compat: .moonlink may be renamed
            const moonLink = line.querySelector('.moonlink') || line.querySelector('a[href*="cp="][class*="moon"]');
            if(!planetLink) return; // v13: skip malformed rows

            planet.id = _cpId(planetLink);
            planet.moonID = _cpId(moonLink);
            planet.coords = _koords(line);
...
GM_getTab(params => unsafeWindow.ogl = new PrOGect(params));
...
class DomManager extends Manager
{
    ...
    {
        this.planet = {};
        // v13 compat: #planetList may be renamed; fall back to common alternatives
        this.planetList = document.querySelector('#planetList') || document.querySelector('#planetlist') || document.querySelector('.planet-list') || document.querySelector('[id*="planetList"]');
        this.smallplanet = document.querySelectorAll('.smallplanet');
        // v13 compat: #links may be renamed
        this.links = document.querySelector('#links') || document.querySelector('.links') || document.querySelector('#sideBar') || document.querySelector('#sidebar');
        // v13 compat: #menuTable may be renamed
        this.leftMenu = document.querySelector('#menuTable') || document.querySelector('#menu') || document.querySelector('.menu_table') || document.querySelector('nav#links');
        // v13 compat: #siteFooter may be renamed
        this.footer = document.querySelector('#siteFooter .fright') || document.querySelector('#siteFooter') || document.querySelector('[id*="siteFooter"]') || document.querySelector('footer');
        // v13 compat: #bar .OGameClock may have moved
        this.mainClock = document.querySelector('#bar .OGameClock') || document.querySelector('.OGameClock') || document.querySelector('#servertime');
        // v13 compat: #countColonies may be renamed
        this.countColonies = document.querySelector('#countColonies') || document.querySelector('[id*="countColonies"]') || document.querySelector('[class*="countColonies"]');

        if(!this.ogl.db.options.displayPlanetTimers && this.planetList)
        {
            this.planetList.classList.add('ogl_alt');
        }

        // v13 compat: helper to safely get planet-koords text (may have changed selector)
        const _getKoords = el =>
        {
            const k = el.querySelector('.planet-koords') || el.querySelector('[class*="koords"]') || el.querySelector('[class*="coords"]');
            return k ? k.textContent.replace(/[\[\]]/g, '').trim() : '';
        };
        // v13 compat: helper to safely get planet name
        const _getName = el =>
        {
            const n = el.querySelector('.planet-name') || el.querySelector('[class*="planet-name"]') || el.querySelector('.planetName');
            return n ? n.textContent.trim() : '';
        };
        // v13 compat: helper to safely extract cp= id from href
        const _getCpId = link =>
        {
            if(!link) return -1;
            const href = link.getAttribute('href') || '';
        ...
        {
            const planet = line.querySelector('.planetlink') || line.querySelector('a[href*="cp="]');
            // v13 compat: .moonlink may be renamed
            const moon = line.querySelector('.moonlink') || line.querySelector('a[href*="cp="][class*="moon"]');
            if(!planet) return; // v13: skip malformed entries

            const planetID = _getCpId(planet);
            const moonID = _getCpId(moon);
            ...
            if(moon)
            {
                ...
                moon._ogl.coords = _getKoords(planet);
        ...
        // v13 compat: #commandercomponent/#lifeform may be renamed
        const LFComponent = document.querySelector('#commandercomponent #lifeform')
            || document.querySelector('[id*="commandercomponent"] #lifeform')
            || document.querySelector('[id*="commandercomponent"] [id*="lifeform"]')
            || document.querySelector('#lifeform');
    ...
    loadFleet()
    {
        ...
        // v13 compat: #fleet1/#fleet2 may be renamed
        this.fleet1 = document.querySelector('#fleet1') || document.querySelector('[id*="fleet1"]') || document.querySelector('[class*="fleet1"]');
        this.fleet2 = document.querySelector('#fleet2') || document.querySelector('[id*="fleet2"]') || document.querySelector('[class*="fleet2"]');
        this.sendAll = document.querySelector('#sendall') || document.querySelector('[id*="sendall"]');
        this.resetAll = document.querySelector('#resetall') || document.querySelector('[id*="resetall"]');
        this.secondCol = document.querySelector('.secondcol') || document.querySelector('[class*="secondcol"]');
        this.warning = this.fleet1?.querySelector('#warning') || this.fleet1?.querySelector('[id*="warning"]');
        this.component = document.querySelector('#fleetdispatchcomponent') || document.querySelector('[id*="fleetdispatch"]');
        this.fleetResources = document.querySelectorAll('#fleet2 .resourceIcon, [id*="fleet2"] .resourceIcon');
        this.requiredShips = Util.addDom('span', { class:'ogl_requiredShips', parent:document.querySelector("#civilships #civil") || document.querySelector('[id*="civilships"] #civil') || this.warning });
    ...
    updateFooter()
    {
        ...
        `
            | <a target="_blank" href="https://ostats.eu/universes/s${this.ogl.server.id}-${this.ogl.server.lang}/player/${this.ogl.account.id}">OStats</a>
            | <a target="_blank" href="https://ogotcha.oplanet.eu/${this.ogl.server.lang}">Ogotcha</a>
            | <a>PrOGect ${this.ogl.version}</a>
        `;
    }
    ...
    {
        // ping
        // v13 compat: #bar may be renamed
        const _barEl = document.querySelector('#bar') || document.querySelector('[id*="bar"].bar') || document.querySelector('#topBar') || document.querySelector('header');
        this.ping = Util.addDom('div', { class:'ogl_ping', parent:_barEl || document.body });
        this.updatePing();
    ...
    updateMiniStats(data)
    {
        ...
        const content = Util.addDom('div', { class:'ogl_miniStatsBody', parent:fragment });

        // 3 main resources stacked vertically, using OGame's OWN flat resource glyphs (.resourceIcon,
        // the game's 48x32 sprite atlas) — NOT OGLight's photographic sprites. deut -> 'deuterium' class.
        // We only set layout in CSS and leave the native background/crop untouched.
        const nativeIcon = { metal:'metal', crystal:'crystal', deut:'deuterium' };
        ['metal', 'crystal', 'deut'].forEach(type =>
        {
            const row = Util.addDom('div', { class:'ogl_miniCell ogl_miniRes', parent:content });
            Util.addDom('div', { class:`resourceIcon ${nativeIcon[type]}`, parent:row });
            Util.addDom('span', { class:`ogl_${type}`, parent:row, child:Util.formatToUnits(data[type], 1, true) });
        });

        // dark matter + artifacts on two columns (OGLight icons kept — the game has no standalone sprites)
        const dual = Util.addDom('div', { class:'ogl_miniDual', parent:content });
        ['dm', 'artefact'].forEach(type =>
        {
            Util.addDom('div', { class:`ogl_miniCell ogl_icon ogl_${type}`, parent:dual, child:`<span>${Util.formatToUnits(data[type], 1, true)}</span>` });
        });

        // MSU on its own full-width row
        Util.addDom('div', { class:'ogl_miniCell ogl_miniMsu ogl_icon ogl_msu', parent:content, child:`<span>${Util.formatToUnits(data.msu, 1, true)}</span>` });
    ...
    updateAvailable()
    {
        Object.values(this.ogl._dom.planet).forEach(planet =>
        {
            ...
            ['metal', 'crystal', 'deut'].forEach(resourceName =>
            {
                ...
                if(storage > 0 && resource >= storage && planet._ogl.type == 'planet') span.classList.add('ogl_danger');
                else if(storage > 0 && resource >= storage * .9 && planet._ogl.type == 'planet') span.classList.add('ogl_warning');
                else
                {
            ...
            this.ogl._dom.redraw(div, fragment, () =>
            {
                if(!planet.classList.contains('tooltipLeft'))
                {
                    if(planet._ogl.type == 'planet')
                    {
                        ...
                        // v13 compat: .planet-koords may be renamed
                        const _kEl = planet.querySelector('.planet-koords') || planet.querySelector('[class*="koords"]') || planet.querySelector('[class*="coords"]');
                        if(_kEl) _kEl.innerHTML = `<span class="ogl_hidden">[</span>${planet._ogl.coords}<span class="ogl_hidden">]</span>`;
                    }
                    else
                    {
...
class LangManager extends Manager
{
    load()
    {
        ...
        this.en =
        {
            ...
            importExportItem:"Show merchant daily item",
            ...
            fleetSave:"Fleet save (night fleet)",
            fleetSaveNoDest:"Fleet Save: set a destination in the settings first",
            fleetSaveConfig:"Fleet save preset",
            expeditionNight:"Night expedition (preset hold time)",
            expeditionHoldTime:"Expedition hold time (h)",
            ...
            manageData:"Manage PrOGect data",
            ...
            resetStatsLong:"Do you really want to reset PrOGect stats data ?",
            resetTaggedPlanetsLong:"Do you really want to reset PrOGect tagged planets data ?",
            resetPinnedPlayersLong:"Do you really want to reset PrOGect pinned players data ?",
            resetAllLong:"Do you really want to reset all PrOGect data ?",
            ...
            siblingShips:"Sibling ships (all but limiter)",
            cargoFallback:"Cargo fallback (if favourite short)",
            oglMessageDone:"This message has been read by PrOGect",
        ...
        this.fr =
        {
            ...
            expeditionValueTT:"<ul><li>La valeur visée par les expédition</li><li>Laisser à <b>0</b> pour utiliser la valeur calculée par PrOGect</li><li>Vous pouvez mettre une valeur fixe pour viser un montant précis</li><li>Vous pouvez utiliser <b>%</b> pour viser un pourcentage de la valeur calculée</li><ul>",
            ...
            menu:"Afficher/masquer le menu PrOGect",
            ...
            manageData:"Gestion des données PrOGect",
            ...
            oglSettings:"Configuration de PrOGect",
            coffee:"Buy me a coffee",
            ...
            resetAll:"Réinitialiser toutes les données PrOGect",
            ...
            resetAllLong:"Voulez-vous vraiment réinitialiser toutes les données PrOGect ?",
            ...
            oglMessageDone:"Ce message a été traité par PrOGect",
        ...
        this.de =
        {
            ...
            manageData:"Verwalte PrOGect Daten",
            ...
            oglSettings:"PrOGect Einstellungen",
            ...
            resetStatsLong:"Möchtest du wirklich die PrOGect Daten zurücksetzen?",
            ...
            resetAllLong:"Möchtest du wirklich alle PrOGect Daten zurücksetzen?",
            ...
            oglMessageDone:"Diese Nachricht wurde von PrOGect gelesen",
        ...
        this.gr =
        {
            ...
            manageData:"Διαχείριση δεδομένων PrOGect",
            ...
            resetStatsLong:"Θέλετε να επαναφέρετε τα δεδομένα των στατιστικών του PrOGect;",
            resetTaggedPlanetsLong:"Θέλετε να επαναφέρετε τα δεδομένα πλανητών με ετικέτα του PrOGect;",
            resetPinnedPlayersLong:"Θέλετε να επαναφέρετε τα δεδομένα των καρφιτσωμένων παικτών του PrOGect;",
            resetAllLong:"Θέλετε να επαναφέρετε όλα τα δεδομένα του PrOGect;",
            ...
            oglMessageDone:"Αυτό το μήνυμα διαβάστηκε από το PrOGect",
...
class TimeManager extends Manager
{
    load()
    {
        ...
        if(this.ogl.page == 'fleetdispatch')
        {
            ...
            // v13 compat: #fleet2/#arrivalTime/#returnTime may be renamed
            document.querySelectorAll('#fleet2 #arrivalTime, #fleet2 #returnTime, [id*="fleet2"] [id*="arrivalTime"], [id*="fleet2"] [id*="returnTime"]').forEach((e, index) =>
            {
        ...
        // v13 compat: #bar ul may be renamed
        const _barUl = document.querySelector('#bar ul') || document.querySelector('[id*="bar"] ul') || document.querySelector('#topBar ul');
        let li = Util.addDom('li', { 'class':'ogl_ping', child:`${ping} s`, parent:_barUl || document.body });
...
class FetchManager extends Manager
{
    ...
    fetchMainData(callback, ignoreDelay)
    {
        ...
        // v13: HighscoreManager only seeds our own score/ranking breakdown when the player
        // manually browses each highscore category page - the Empire summary tab was showing
        // "0 pts"/undefined for everything except global rank. fetchPlayerAPI already fetches
        // this exact data (players.xml, public API, 7-day cache) but was previously only called
        // reactively (clicking other players); call it once here for our own account too.
        this.fetchPlayerAPI(this.ogl.account.id, this.ogl.account.name, () => { if(this.ogl._account) this.ogl._account.calcScore(); });

        Promise.all([
            this.fetchLFBonuses(false, ignoreDelay),
            this.fetchAccountInfo(ignoreDelay),
    ...
    // v13: OGame's official per-account data export API (see AGENTS.md SS4.1/SS6 - public,
    // documented endpoint, not a private/report API). Compliance: called ONLY from real
    // page-load-triggered paths (fetchMainData, on demand from the empire page) via the
    // existing pending-queue/setTimeout bootstrap - never on a setInterval/polling loop, and
    // never with cp= in the request. Response covers ALL planets+moons in one call, so this
    // fully replaces the old (dead) per-type empire endpoint scraper below.
    async fetchAccountInfo(force)
    {
        // v12: the accountInfo export endpoint does not exist there, so the empire update used to do
        // nothing at all on pre-v13 servers. Fall back to the per-body-type empire ajax endpoint, which
        // feeds EmpireManager.update() - the v12 parser we already ship and which was simply never called.
        if(this.ogl.server.gameVersion < 13)
        {
            await Promise.all([this.fetchAccountV12(0, force), this.fetchAccountV12(1, force)]);
            return null;
        }

        // in-memory cache so repeated calls within the same page-load cycle reuse one fetch
        if(!force && this._accountInfoCache && Date.now() - (this._accountInfoCacheAt || 0) < 120000)
        {
            return this._accountInfoCache;
        }

        // capability probe: this endpoint does not exist on pre-v13 servers; remember a
        // negative result for the session so we do not retry it on every page load
        if(sessionStorage.getItem('syl-dataexport-available') === '0') return null;
        ...
        try
        {
            ...
            const response = await fetch(url, { credentials:'include', cache:'no-cache', headers: { 'X-Requested-With':'XMLHttpRequest' } });
            ...
            {
                // 200 with non-JSON body ("An error has occured!") = endpoint absent on this server
                sessionStorage.setItem('syl-dataexport-available', '0');
                return null;
            }
            sessionStorage.setItem('syl-dataexport-available', '1');

            // propagate the rotated ajax token so we do not desync OGame's own requests
            if(json.newAjaxToken && typeof unsafeWindow.setNewTokenData === 'function')
            {
                try { unsafeWindow.setNewTokenData(json.newAjaxToken); }
                catch(e) {}
            }

            this._accountInfoCache = json;
            this._accountInfoCacheAt = Date.now();

            this.applyAccountInfo(json);
    ...
    // v12 only: page=ajax&component=empire returns { mergedArray:"<json string>" } for one planetType
    // (0 = planets, 1 = moons), so the payload needs a second JSON.parse. Two calls (planets + moons) on
    // the same page-load path as the v13 fetch - no timer, no polling, no cp= (AGENTS.md SS4). The
    // AbortController setTimeout is a request timeout, not a poll.
    async fetchAccountV12(type, force)
    {
        type = type || 0;
        ...
        if(!force && Date.now() < this.ogl.db[`lastEmpire${type}Update`] + 60000 * 3) { clearTimeout(timeout); return null; }   // 3mn

        try
        {
            const response = await fetch(`https://${window.location.host}/game/index.php?page=ajax&component=empire&ajax=1&planetType=${type}&asJson=1`,
            {
                credentials:'include',
        ...
        {
            clearTimeout(timeout);
            console.error('PrOGect: cannot fetch v12 empire data', e);
            return null;
        }
    }

    // v13: OGame's official daily merchant offer (externaldataexport action=importExportInfo).
    // Feeds the "which item is today's offer" badge on the Merchant button so the player knows
    // the item WITHOUT opening the merchant. Compliance: ONE page-load call (from fetchMainData),
    // cached in db.importExportOffer, never polled, never with cp= (AGENTS.md §4.1/§6). It only
    // reads data the game already exposes in the merchant - no premium-feature imitation (§1.6).
    async fetchImportExportInfo(force)
    {
        // reuse the accountInfo capability probe: same component, absent on pre-v13 servers
        if(sessionStorage.getItem('syl-dataexport-available') === '0') return null;
        // one fetch per page-load cycle - the offer only changes once a day
        if(!force && this._importExportFetched) return this.ogl.db.importExportOffer || null;

        try
        {
            const url = `https://${window.location.host}/game/index.php?page=componentOnly&component=externaldataexport&action=importExportInfo&asJson=1`;
            const response = await fetch(url, { credentials:'include', cache:'no-cache', headers: { 'X-Requested-With':'XMLHttpRequest' } });
            if(!response.ok) return null;
            ...
            catch(e) { sessionStorage.setItem('syl-dataexport-available', '0'); return null; }
            sessionStorage.setItem('syl-dataexport-available', '1');
            ...
            this._importExportFetched = true;
            this.ogl.db.importExportOffer =
            {
                name: json.name,
                rarity: json.rarity,
                price: json.price,
                hasBought: json.hasBought,
                gotItem: json.gotItem,
                offersLeft: json.offersLeft,
                itemText: json.itemText,
                bargainText: json.bargainText,
                fetchedAt: serverTime.getTime()
            };
            this.ogl.save();

            return this.ogl.db.importExportOffer;
        }
        catch(e) { return null; }
    }

    applyAccountInfo(json)
    {
        const allowedEntries = ['metal', 'crystal', 'deuterium', 'energy', 'food', 'population'];
        // v13: research levels are ACCOUNT-WIDE (top-level json.researches, not per-planet) but
        // the UI (AccountManager.openEmpire, TechManager) reads them as flat myPlanets[id][techId]
        // properties, same as buildings/ships - the old empire endpoint used to replicate them onto
        // every planet entry for that reason. Without this, research score/levels never populate.
        const researches = json.researches || {};

        const applyBody = (id, body, type) =>
        {
            this.ogl.db.myPlanets[id] = this.ogl.db.myPlanets[id] || {};
            const planet = this.ogl.db.myPlanets[id];
            planet.type = type;
            planet.id = planet.id || id;
            if(body.siblingId) planet[type === 'planet' ? 'moonID' : 'planetID'] = body.siblingId;
            if(body.galaxy != null && body.system != null && body.position != null) planet.coords = `${body.galaxy}:${body.system}:${body.position}`;
            ...
            // TechManager/AccountManager
            [body.buildings, body.ships, body.defenses, body.speciesBuildings, body.speciesResearches, researches].forEach(map =>
            {
                if(!map) return;
                Object.entries(map).forEach(entry => { planet[entry[0]] = entry[1]; });
            });

            planet.lastRefresh = serverTime.getTime();
        };

        Object.entries(json.planets || {}).forEach(entry => applyBody(entry[0], entry[1], 'planet'));
        Object.entries(json.moons || {}).forEach(entry => applyBody(entry[0], entry[1], 'moon'));

        if(this.ogl._ui) Util.runAsync(() => this.ogl._ui.displayResourcesRecap());
        if(this.ogl._dom?.updateAvailable) this.ogl._dom.updateAvailable();
    }

    // v13: accountInfo does not expose fieldUsed/fieldMax/temperature at all - confirmed against
    // the official field list (OGame's own export API omits them). The empire page's native HTML
    // (page=standalone&component=empire) embeds the full per-planet data as a JSON object passed
    // to createImperiumHtml({planets, groups, ...}) - fieldUsed/fieldMax/db_par2(=min temperature)
    // live there. This is fetched purely to backfill that one gap; resources/production/buildings/
    // ships/defenses still come from accountInfo. Cached for 24h since fields/temperature are
    // effectively static (temperature never changes; fields only grow when the player builds).
    async fetchEmpireFields(planetType, force)
    {
        planetType = planetType || 0;
        const cacheKey = `lastEmpireFieldsUpdate${planetType}`;
        if(!force && this.ogl.db[cacheKey] && Date.now() < this.ogl.db[cacheKey] + 86400000) return;

        try
        {
            const url = `https://${window.location.host}/game/index.php?page=standalone&component=empire&planetType=${planetType}`;
            const response = await fetch(url, { credentials:'include', cache:'no-cache' });
            if(!response.ok) return;
            const html = await response.text();

            const markerIdx = html.indexOf('createImperium');
            if(markerIdx < 0) return;
            const parenIdx = html.indexOf('(', markerIdx);
            if(parenIdx < 0) return;
            const braceIdx = html.indexOf('{', parenIdx);
            if(braceIdx < 0) return;

            // extract the balanced {...} object, respecting quoted strings so a "}" inside a
            // translation string does not close the object early
            let depth = 0, inStr = false, quote = '', endIdx = -1;
            ...
            {
                const id = p.id;
                if(!id || !this.ogl.db.myPlanets[id]) return;
                const planet = this.ogl.db.myPlanets[id];
                if(p.fieldUsed != null) planet.fieldUsed = p.fieldUsed;
                if(p.fieldMax != null) planet.fieldMax = p.fieldMax;
                if(p.db_par2 != null) planet.temperature = p.db_par2;
            });

            this.ogl.db[cacheKey] = Date.now();

            if(this.ogl._ui) Util.runAsync(() => this.ogl._ui.displayResourcesRecap());
        }
        catch(e)
        {
            console.warn('PrOGect: fetchEmpireFields failed', e);
        }
    }
    ...
    async fetchLFBonuses(callback, ignoreDelay)
    {
        ...
        // v13: read the bonuses from OGame's own species-bonuses export instead of scraping the lfbonuses
        // page. That page keeps its values inside custom elements (<bonus-item>), and a DOMParser document
        // never renders custom elements, so the scrape returned empty rows: LfResearch.duration ended up
        // missing and the (1 - bonus.duration) factor collapsed to 1, making every research / LF-research
        // duration far too long. The export returns the same totals as plain JSON.
        // Compliance: one call on the existing page-load path, read-only, no cp= (AGENTS.md SS4/SS6).
        if(this.ogl.server.gameVersion >= 13)
        {
            const v13Controller = new AbortController();
            const v13Timeout = setTimeout(() => v13Controller.abort(), 10000);   // request timeout, not a poll

            try
            {
                const response = await fetch(`https://${window.location.host}/game/index.php?page=componentOnly&component=externaldataexport&action=speciesBonuses&asJson=1`,
                {
                    credentials:'include',
                    headers: { 'X-Requested-With':'XMLHttpRequest' },
                    signal: v13Controller.signal
                });
                clearTimeout(v13Timeout);
                if(!response.ok) return;

                this.ogl._empire?.updateLFBonuses?.(await response.json());
                this.ogl.db.lastLFBonusUpdate = Date.now();
                this.refreshLFDependentUI();
                if(callback) callback();
            }
            catch(e)
            {
                clearTimeout(v13Timeout);
                console.error('PrOGect: cannot fetch species bonuses', e);
            }
        ...
        {
            clearTimeout(timeout);
            // v13 fix: test the Response's .ok/.status BEFORE reading the body. The old code
            // did response.text() first, then checked `.ok` on the resulting STRING (always
            // undefined) -> the standalone fallback fired on every call, silently double-fetching
            // the whole LF page each time. Now we branch on the real Response object.
            if(response.ok) return response.text();
            console.warn(`PrOGect LF: ajax status ${response.status}, trying standalone`);
            return fetch(`https://${window.location.host}/game/index.php?page=standalone&component=lfbonuses`,
            {
                headers: { 'X-Requested-With': 'XMLHttpRequest' },
                signal: controller.signal
            }).then(r => r.text());
        ...
        .then(result =>
        {
            ...
            const hasBonusItem = xml.querySelector('bonus-item-content-holder, bonus-item');
            const hasWrapper = xml.querySelector('.lfsettingsContentWrapper');
            const hasLfItem = xml.querySelector('lifeform-item, .lifeform-item');

            if(!hasWrapper && !hasBonusItem && !hasLfItem)
            {
                console.warn('PrOGect LF: no recognizable LF bonus content in response');
                return;
            }


            if(this.ogl._empire) this.ogl._empire.getLFBonuses(xml);
            this.ogl.db.lastLFBonusUpdate = Date.now();
    ...
    // Shared by both LF-bonus paths (v13 JSON + v12 scrape): the expedition maximum and the fleet page
    // depend on the freshly loaded bonuses, so refresh whatever is currently on screen.
    refreshLFDependentUI()
    {
        document.querySelectorAll('[data-allowPercent] .ogl_placeholder, input.ogl_placeholder[data-allowPercent]').forEach(input =>
        {
            input.setAttribute('placeholder', `(${Util.formatNumber(this.ogl.calcExpeditionMax().max)})`);
        });

        if(this.ogl.page === 'fleetdispatch' && this.ogl._fleet)
        {
            try { this.ogl._fleet.checkSendShips(); } catch(e) {}
        }
    }
...
class UIManager extends Manager
{
    load(reloaded)
    {
        ...
        if(!reloaded)
        {
            // v13 compat: countColonies or mainClock may be null
            if(this.ogl._dom.mainClock && this.ogl._dom.countColonies)
            {
        ...
        if(!reloaded)
        {
            ...
            document.querySelectorAll('.planetlink, .moonlink').forEach(target =>
            {
                target.addEventListener('pointerenter', event =>
                {
                    ...
                    // v13 compat: robust planet-koords and icon selectors
                    const _sp = event.target.closest('.smallplanet');
                    if(!_sp) return;
                    const _ke = _sp.querySelector('.planet-koords') || _sp.querySelector('[class*="koords"]') || _sp.querySelector('[class*="coords"]');
                    const coords = _ke ? _ke.innerText.replace(/[\[\]]/g, '').split(':') : [];
                    if(coords.length < 3) return;
                    // v13 compat: moonlink class may be renamed
                    const type = (event.target.classList.contains('moonlink') || event.target.closest('[class*="moon"]')?.closest('.smallplanet')) ? 3 : 1;
                    const name = type == 3
                        ? (_sp.querySelector('.icon-moon, .moonlink img')?.getAttribute('alt') || '')
                        : (_sp.querySelector('.planetPic, .planetlink img')?.getAttribute('alt') || '');
                ...
                target.addEventListener('pointerleave', () =>
                {
                    ...
                    // v13 compat: robust galaxy/system/position input selectors
                    const _gl = document.querySelector('#galaxy') || document.querySelector('[name="galaxy"]');
                    const _sl = document.querySelector('#system') || document.querySelector('[name="system"]');
                    const _pl = document.querySelector('#position') || document.querySelector('[name="position"]');
                    if(_gl) _gl.value = fleetDispatcher.realTarget.galaxy;
                    if(_sl) _sl.value = fleetDispatcher.realTarget.system;
                    if(_pl) _pl.value = fleetDispatcher.realTarget.position;
            ...
            document.addEventListener('click', event =>
            {
                ...
                // v13 compat: in v13 the DOM has more inner wrappers (ogl_unit/ogl_suffix spans), so event.target
                // is often a child of the [data-galaxy] container. Walk up to find the ancestor with the attribute.
                const galaxyEl = event.target.closest && event.target.closest('[data-galaxy]');
                const rawCoords = galaxyEl ? galaxyEl.getAttribute('data-galaxy') : (event.target.getAttribute && event.target.getAttribute('data-galaxy'));
                ...
                if(rawCoords)
                {
                    ...
                    if(this.ogl.page === 'galaxy')
                    {
                        ...
                        // v13 compat: loadContentNew may not exist as a global anymore
                        if(typeof loadContentNew === 'function') loadContentNew(galaxy, system);
                        else if(unsafeWindow.loadContentNew) unsafeWindow.loadContentNew(galaxy, system);
                        else
                        {
                            const url = `https://${window.location.host}/game/index.php?page=ingame&component=galaxy&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2] || 1}`;
                ...
                || (event.target.closest('a[href*="cp="]') && event.target.closest('.smallplanet')))
                {
                    if(document.body.classList.contains('ogl_destinationPicker'))
                    {
                        // v13: only intercept if we are actually in fleet dispatch mode
                        if(!unsafeWindow.fleetDispatcher && !document.body.classList.contains('ogl_initHarvest') && !document.body.classList.contains('ogl_initShipHarvest'))
                        {
                            // not in fleet context - remove stale ogl_destinationPicker and allow normal navigation
                            document.body.classList.remove('ogl_destinationPicker');
                            document.body.classList.remove('ogl_initHarvest');
                            document.body.classList.remove('ogl_initShipHarvest');
                            return;
                        ...
                        // v13 compat: robust planet-koords selector
                        const _smallplanet = event.target.closest('.smallplanet');
                        if(!_smallplanet) return;
                        const _koordsEl = _smallplanet.querySelector('.planet-koords') || _smallplanet.querySelector('[class*="koords"]') || _smallplanet.querySelector('[class*="coords"]');
                        const coords = _koordsEl ? _koordsEl.innerText.replace(/[\[\]]/g, '').split(':') : [];
                        if(coords.length < 3) return;
                        // v13 compat: moonlink/planetlink class may be renamed
                        const _closestPL = event.target.closest('.planetlink, .moonlink, a[href*="cp="]');
                        const type = (_closestPL?.classList.contains('moonlink') || (_closestPL?.closest('[class*="moon"]') && _closestPL?.closest('.smallplanet'))) ? 3 : 1;
                        // v13: icon-moon or moonlink img alt may have changed; use safe fallback
                        const _moonAlt = _smallplanet.querySelector('.icon-moon, .moonlink img')?.getAttribute('alt') || '';
                        const _planetAlt = _smallplanet.querySelector('.planetPic, .planetlink img')?.getAttribute('alt') || '';
                        const name = type == 3 ? _moonAlt : _planetAlt;
                        const planet = this.ogl.account.planets.getByCoords(coords.join(':'));
                        
                        const id = type == 3 ? planet.moonID : planet.id;

                        if(document.body.classList.contains('ogl_initHarvest') || document.body.classList.contains('ogl_initShipHarvest'))
                        {
                            ...
                            // ships collect (oglmode 7) vs resources collect (oglmode 1); the picked body is
                            // the FIXED destination, the source bodies cycle. selectSiblingShips fills ships.
                            const oglmode = document.body.classList.contains('ogl_initShipHarvest') ? 7 : 1;

                            window.location.href = `https://${window.location.host}/game/index.php?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${type}&oglmode=${oglmode}&ogldestinationid=${id}&oglfirstsourceid=${firstSourceId}&ogldestinationtype=${type == 3 ? 'moon' : 'planet'}`;
                        }
                        else
                        {
                ...
                {
                    // v13 compat: getCurrentMessageTab may not exist; fallback to tabID
                    const _curTab = (typeof unsafeWindow.ogame?.messages?.getCurrentMessageTab === 'function')
                        ? unsafeWindow.ogame.messages.getCurrentMessageTab()
                        : (this.ogl._message?.tabID ?? -1);
                    if(_curTab == 20)
                    {
                        ogl.cache.reports = {};
                    }
    ...
    openFleetProfile(fromSettings)
    {
        ...
        resourceKeys.forEach(addRow);
        Util.addDom('div', { class:'ogl_limiterSep', parent:grid });
        shipKeys.forEach(addRow);
    ...
    // PrOGect: keyboard settings restored to OGLight's ORIGINAL renderer — plain key inputs + one
    // Save button (no custom chips, no "reset" button). Lists every binding in keyboardActions.
    buildKeyboardUI()
    {
        const container = Util.addDom('div', { class:'ogl_keyboardActions' });
        const changes = {};
        ...
        Object.entries(this.ogl.db.options.keyboardActions).forEach(key =>
        {
            const label = Util.addDom('label', { parent:container, child:`${this.ogl._lang.find(key[0])}` });
            const input = Util.addDom('input', { maxlength:'1', type:'text', value:key[1], parent:label,
                onclick:() => { input.value = ''; input.select(); },
                onblur:() => { if(input.value == '') input.value = key[1]; },
                onkeyup:event =>
                {
                    if(event.key.toLowerCase() == 'enter') return;
                    changes[key[0]] = event.key.toLowerCase();
            ...
            if(key[0] == 'fleetResourcesSplit') { input.classList.add('ogl_disabled'); input.disabled = true; }
        });

        Util.addDom('button', { parent:container, class:'ogl_button', child:'save', onclick:() =>
        {
            Object.entries(changes).forEach(key => { this.ogl.db.options.keyboardActions[key[0]] = changes[key[0]]; window.location.reload(); });
        }});

        return container;
    }

    // PrOGect: NEW allowed-biggest-ships UI — toggle ship cards; writes db.options.expeditionBigShips.
    buildBiggestShipsUI()
    {
        const container = Util.addDom('div', { class:'ogl_sylPanel', child:'<h2><i class="material-icons">rocket_launch</i> Allowed biggest ships</h2>' });
        const grid = Util.addDom('div', { class:'ogl_sylShipGrid', parent:container });

        [204, 205, 206, 207, 215, 211, 213, 218].forEach(shipID =>
        {
            const on = this.ogl.db.options.expeditionBigShips.indexOf(shipID) > -1;
            const card = Util.addDom('button', { class:'ogl_sylShipCard ogl_button'+(on ? ' ogl_active' : ''), parent:grid, child:`<div class="ogl_icon ogl_${shipID}"></div><span>${this.ogl._lang.find(shipID, true)}</span>`, onclick:() =>
            {
                const idx = this.ogl.db.options.expeditionBigShips.indexOf(shipID);
                if(idx > -1) { this.ogl.db.options.expeditionBigShips.splice(idx, 1); card.classList.remove('ogl_active'); }
                else { this.ogl.db.options.expeditionBigShips.push(shipID); card.classList.add('ogl_active'); }
            }});
        });
    ...
    openDataManager(fromSettings)
    {
        const container = Util.addDom('div', {class:'ogl_manageData'});
        Util.addDom('h2', { parent:container, child:this.ogl._lang.find('manageData') });

        // Back button only when opened from settings (returns to the main settings). Opened as a standalone
        // popup so it isn't width-mangled by the big-settings detail layout.
        if(fromSettings) Util.addDom('div', { class:'ogl_panelBack ogl_button tooltip', title:'Back to settings', parent:container, child:'<i class="material-icons">arrow_back</i>', onclick:() => this.ogl._topbar.openSettings() });
        ...
        // export
        Util.addDom('a', { class:'ogl_button', download:`progect_${this.ogl.server.name}_${this.ogl.server.lang}_${serverTime.getTime()}`, child:'<i class="material-icons">file_export</i>Export data', parent:grid, href:URL.createObjectURL(new Blob([JSON.stringify(this.ogl.db)], {type: 'application/json'})) });
    ...
    groupPlanets()
    {
        ...
        {
            // v13 compat: .planet-koords may be renamed
            const _koordsElGP = planet.querySelector('.planet-koords') || planet.querySelector('[class*="koords"]') || planet.querySelector('[class*="coords"]');
            let newCoords = Util.coordsToID(_koordsElGP?.innerText || '0:0:0').slice(0, -3);
    ...
    checkImportExport()
    {
        const btn = document.querySelector('.menubutton[href*=traderOverview], .menubutton[href*=trader], a[data-component=traderOverview]');
        ...
        // PrOGect: reveal WHICH item today's merchant offer is, next to the button, from the
        // official importExportInfo export API. Compliance (AGENTS.md §1.7): the badge is ADDITIVE
        // and adjacent - it never hides, moves, resizes, reskins or changes the opacity of the
        // Merchant button itself. The "show/hide" is options.importExportItem (Settings > Interface).
        this.renderImportExportItem(btn);
        // single page-load fetch to refresh the cached offer, then re-render (no polling, no cp=)
        this.ogl._fetch.fetchImportExportInfo().then(() => this.renderImportExportItem(btn));
    ...
    // PrOGect: decorates the Merchant button with a subtle rarity-colored ring for today's offer.
    // ADDITIVE only — it never hides, obscures, resizes, moves or reskins the Merchant button or any
    // monetization content (AGENTS.md §1.7). Shown only when the offer is available, opted in
    // (options.importExportItem) AND not yet redeemed. No fetch here (reads the cached offer).
    renderImportExportItem(btn)
    {
        btn = btn || document.querySelector('.menubutton[href*=traderOverview], .menubutton[href*=trader], a[data-component=traderOverview]');
        const li = btn ? btn.closest('li') : null;
        if(!li) return;

        // clear previous decoration (offer / availability may have changed)
        li.classList.remove('ogl_ieHost');
        li.removeAttribute('data-ie-rarity');
        li.onmouseenter = null;
        li.onmouseleave = null;
        document.querySelector('.ogl_ieTip')?.remove();             // drop any tooltip from an older build
        li.parentNode?.querySelector('.ogl_ieItemRow')?.remove();   // drop the old (pre-ring) menu row if present

        const offer = this.ogl.db.importExportOffer;
        const available = (this.ogl.db.nextImportExport || 0) < serverTime.getTime();
        if(!(this.ogl.db.options.importExportItem && available && offer && offer.name)) return;

        // the export API returns hasBought/gotItem as the STRINGS "true"/"false" - parse them.
        // once the offer has been redeemed, show NOTHING (no ring).
        const taken = offer.gotItem === true || offer.gotItem === 'true' || offer.hasBought === true || offer.hasBought === 'true';
        if(taken) return;

        li.classList.add('ogl_ieHost');
        if(offer.rarity != null) li.setAttribute('data-ie-rarity', String(offer.rarity).toLowerCase());
    }
    ...
    getPlayerTooltip(id)
    {
        ...
        const container = Util.addDom('div',
        {
            ...
            child:`
            <h1 class="${status}">${player.name} <a href="https://${window.location.host}/game/index.php?page=ingame&component=highscore&site=${page}&category=1&searchRelId=${id}">#${player.score.globalRanking}</a></h1>
            <div class="ogl_grid">
        ...
        {
            window.open(Util.genOstatsLink(this.ogl, player.uid), '_blank');
        }});
...
class TopbarManager extends Manager
{
    load(reloaded)
    {
        ...
        Util.addDom('i', {class:'material-icons tooltip ogl_tbHarvest', child:'conversion_path', title:this.ogl._lang.find('collectResources'),parent:this.topbar, onclick:e =>
        {
            ...
            document.body.classList.remove('ogl_initShipHarvest');
            const active = document.body.classList.toggle('ogl_initHarvest');
            document.body.classList.toggle('ogl_destinationPicker', active);
        }});

        // PrOGect: sibling (ships) — sits next to the resource-collect (harvest) icon. Clicking it
        // enters a destination picker: pick ONE moon/planet in the side list, then all ships (minus the
        // limiter) are sent there, cycling through the source bodies while keeping that fixed destination
        // (oglmode=7 - same mechanism as the resource collect, ships instead of cargo). Each send stays a
        // manual Enter (1 keystroke = 1 send, AGENTS.md §1.1).
        Util.addDom('i', { class:'material-icons tooltip ogl_tbSiblingShips', child:'rocket_launch', parent:this.topbar, title:this.ogl._lang.find('siblingShips'), onclick:() =>
        {
            this.ogl.db.harvestCoords = false;
            document.body.classList.remove('ogl_initHarvest');
            const active = document.body.classList.toggle('ogl_initShipHarvest');
            document.body.classList.toggle('ogl_destinationPicker', active);
        }});

        Util.addDom('i', { class:'material-icons tooltip ogl_tbEmpire', child:'account_balance', parent:this.topbar, title:this.ogl._lang.find('accountSummary'), onclick:() =>
        {
        ...
        Util.addDom('i', { class:'material-icons tooltip ogl_tbTagged', child:'stroke_full', parent:this.topbar, title:this.ogl._lang.find('taggedPlanets'), onclick:() => this.openTagged(true) });
        Util.addDom('i', { class:'material-icons tooltip ogl_tbPinned', child:'keep', parent:this.topbar, title:this.ogl._lang.find('pinnedPlayers'), onclick:() => this.openPinned(true) });
        Util.addDom('i', { class:'material-icons tooltip ogl_tbSettings', child:'settings', parent:this.topbar, title:this.ogl._lang.find('oglSettings'), onclick:() => this.openSettings(true) });
        
        this.syncBtn = Util.addDom('i', {class:'material-icons tooltip ogl_tbSync', child:'directory_sync', title:this.ogl._lang.find('syncEmpire'), parent:this.topbar, onclick:() =>
        {
            this.ogl._fetch.fetchMainData(false, true);
        }});

        // PrOGect: global night-fleet-save button — navigates to the fleet page with oglmode=6, where
        // fleetSave() pre-fills the preset and the player confirms the send (1 click flow, AGENTS.md 1.1).
        Util.addDom('i', { class:'material-icons tooltip ogl_tbFleetSave', child:'bedtime', parent:this.topbar, title:this.ogl._lang.find('fleetSave'), onclick:() =>
        {
            window.location.href = `https://${window.location.host}/game/index.php?page=ingame&component=fleetdispatch&oglmode=6`;
        }});
    ...
    openAccount()
    {
        ...
        document.querySelectorAll('.smallplanet').forEach(line =>
        {
            ...
            // v13 compat: .planet-name may be renamed
            const _nameEl = line.querySelector('.planet-name') || line.querySelector('[class*="planet-name"]') || line.querySelector('.planetName');
            const name = _nameEl?.innerText || '';
            ...
            // planet picture
            // v13 compat: .planetlink/.planetPic may be renamed
            const _planetLinkEl = line.querySelector('.planetlink') || line.querySelector('a[href*="cp="]:not([class*="moon"])');
            const _planetPicEl = line.querySelector('.planetPic') || line.querySelector('.planetlink img') || line.querySelector('a[href*="cp="] img');
            Util.addDom('a', 
            {
                parent:grid,
                href:_planetLinkEl?.getAttribute('href') || '#',
                child:Util.addDom('img',
                {
                    src:_planetPicEl?.getAttribute('src') || ''
                })
            });

            // moon picture
            // v13 compat: .moonlink may be renamed
            const _moonLinkEl = line.querySelector('.moonlink') || line.querySelector('a[href*="cp="][class*="moon"]');
            if(_moonLinkEl)
            {
                Util.addDom('a', 
                {
                    parent:grid,
                    href:_moonLinkEl.getAttribute('href'),
                    child:Util.addDom('img',
                    {
                        src:(_moonLinkEl.querySelector('img') || _moonLinkEl)?.getAttribute('src') || ''
                    })
                });
            }
            else
    ...
    openSettings(buttonSource)
    {
        ...
        // PrOGect: inline detail view. Sub-configs (limiter/fleet profile, allowed biggest ships,
        // fleet save, keyboard, data) render INSIDE this same popup with a Back button, instead of
        // opening a SECOND popup on the same layer — which used to clobber the settings and close
        // everything when the inner one was dismissed.
        const detail = Util.addDom('div', { class:'ogl_settingsDetail' });
        const openDetail = (node, title) =>
        {
            ...
            container.classList.add('ogl_showDetail');
            body.querySelectorAll('.ogl_inputCheck').forEach(e => Util.formatInput(e));
        };
        ...
        options.forEach(opt =>
        {
            ...
            if(opt == 'defaultShip')
            {
                this.ogl.fretShips.forEach(shipID =>
                {
                    let div = Util.addDom('div', {parent:label, class:`ogl_icon ogl_${shipID}`, onclick:(event, element) =>
                    {
                        ...
                        if(this.ogl.page == 'fleetdispatch')
                        {
                            // v13 compat: #fleet1 may be renamed
                            document.querySelectorAll('#fleet1 .ogl_fav, [id*="fleet1"] .ogl_fav').forEach(e => e.classList.add('ogl_grayed'));
                            ...
                            if(this.ogl.mode === 1 || this.ogl.mode === 2)
                            {
                                ...
                                this.ogl._fleet.selectCargoShips();
            ...
            else if(opt == 'profileButton')
            {
                ...
                {
                    Util.runAsync(() => this.ogl._ui.openFleetProfile(true)).then(e => this.ogl._popup.open(e));
                });
            }
            else if(opt == 'fleetSaveConfig')
            {
                label.innerHTML = `<button class="material-icons">bedtime</button>`;
                label.querySelector('button').addEventListener('click', () => openDetail(this.openFleetSaveConfig(), this.ogl._lang.find('fleetSaveConfig')));
            }
            else if(opt == 'msu')
            {
                label.classList.add('tooltipLeft');
            ...
            else if(opt == 'cargoFallback')
            {
                ...
                ['Off', 'Other cargo', 'Other cargo + Pathfinder', 'Other cargo + Recycler'].forEach((entry, index) =>
                {
            ...
            else if(opt == 'showMenuResources')
            {
                ...
                {
                    this.ogl.db.options[opt] = parseInt(select.value);
                    localStorage.setItem('syl_menulayout', this.ogl.db.options[opt]);
            ...
            {
                // A palette picker rather than an on/off switch: the three common deficiencies confuse
                // different pairs, so one "colorblind" palette cannot serve them all. Deuteranopia and
                // protanopia both confuse red with green but differ in how dark reds read, while
                // tritanopia keeps red/green and confuses blue with green and yellow with violet - the
                // opposite trade. Achromatopsia has no hue at all, so that palette works on lightness.
                label.innerHTML = ``;
                const select = Util.addDom('select', { parent:label, class:'dropdownInitialized', onchange:() =>
                {
                    this.ogl.db.options[opt] = select.value;
                    localStorage.setItem('syl_colorblind', select.value);
                    document.body.setAttribute('data-colorblind', select.value);
                }});

                [['', 'Off'], ['deuteranopia', 'Deuteranopia (red-green)'], ['protanopia', 'Protanopia (red-green)'], ['tritanopia', 'Tritanopia (blue-yellow)'], ['achromatopsia', 'Achromatopsia (no colour)']].forEach(entry =>
                {
                    const selectOption = Util.addDom('option', { parent:select, child:entry[1], value:entry[0] });
            ...
            else if(isBoolean && opt != 'sim' && opt != 'expeditionValue')
            {
                ...
                // PrOGect: real toggle-switch component (track + hidden functional input + knob).
                // The <input> keeps the ORIGINAL onclick verbatim, so every side effect is preserved.
                const sw = Util.addDom('span', { class:'ogl_sylToggle', parent:label });
                const input = Util.addDom('input', { type:'checkbox', class:'ogl_sylToggleInput', parent:sw, onclick:() =>
                {
                    ...
                    {
                        // v13 compat: #planetList may be renamed
                        (document.querySelector('#planetList') || document.querySelector('[id*="planetList"]') || document.querySelector('.planet-list'))?.classList.toggle('ogl_alt')
                    }
                    else if(opt == 'reduceLargeImages')
                    {
                        localStorage.setItem('syl_minipics', this.ogl.db.options[opt]);
                ...
                Util.addDom('span', { class:'ogl_sylToggleKnob', parent:sw });
            ...
            else if(opt == 'keyboardActions')
            {
                ...
                label.querySelector('button').addEventListener('click', () =>
                {
                    Util.runAsync(() => this.ogl._ui.buildKeyboardUI()).then(e => openDetail(e, this.ogl._lang.find('keyboardActions')));
                });
            }
            ...
            else if(opt == 'expeditionBigShips')
            {
                ...
                label.querySelector('button').addEventListener('click', () =>
                {
                    Util.runAsync(() => this.ogl._ui.buildBiggestShipsUI()).then(e => openDetail(e, this.ogl._lang.find('expeditionBigShips')));
                });
            }
            ...
            else if(opt == 'spyTableRules')
            {
                ...
                label.querySelector('button').addEventListener('click', () =>
                {
                    Util.runAsync(() => this.ogl._message.buildSpyTableRules()).then(e => openDetail(e, this.ogl._lang.find('spyTableRules')));
                });
            }
            else if(opt == 'ptreTeamKey')
            {
                ...
                const input = Util.addDom('input', { type:'password', placeholder:'TM-XXXX-XXXX-XXXX-XXXX', value:localStorage.getItem('syl-ptreTK') || '', parent:label,
                oninput:() =>
                {
                    ...
                    {
                        input.classList.remove('ogl_danger');
                        localStorage.setItem('syl-ptreTK', input.value);
            ...
            else if(opt == 'ptreLogs')
            {
                ...
                label.querySelector('button').addEventListener('click', () =>
                {
                    Util.runAsync(() => this.ogl.PTRE.buildLogs()).then(e => openDetail(e, this.ogl._lang.find('ptreLogs')));
                });
            }
            else if(opt == 'manageData')
            {
                ...
                label.querySelector('button').addEventListener('click', () =>
                {
                    Util.runAsync(() => this.ogl._ui.openDataManager(true)).then(e => this.ogl._popup.open(e));
                });
            }
        });

        // PrOGect: OGame-Infinity-style big settings page — every category section is expanded
        // and laid out across columns (no accordion), on one scrollable page. Reuses the existing
        // option DOM/logic; sub-config launchers still open the inline detail.
        container.classList.add('ogl_bigSettings');
        const grid = Util.addDom('div', { class:'ogl_settingsGrid' });
        container.querySelectorAll('[data-container]').forEach(sec =>
        {
            sec.classList.add('ogl_active');            // always expanded
            const h3 = sec.querySelector('h3');
            if(h3)
            {
                // native OGLight section icon comes from h3::before (its own icon font) — keep it
                h3.onclick = null; h3.style.cursor = 'default'; // no collapse toggle
            }
            grid.appendChild(sec);                       // move section into the multi-column grid
        });
        container.appendChild(grid);

        // PrOGect: donation link — moved out of the topbar into settings. Optional Ko-fi support
        // (allowed per AGENTS.md §1.8: a donation button is fine; no paywall, fees, or locked features).
    ...
    openFleetSaveConfig(forBodyId)
    {
        ...
        {
            const row = Util.addDom('label', { parent:container, 'data-label':labelText });
            const input = Util.addDom('input', { type:'text', value:(opt[key] != null ? opt[key] : ''), placeholder:ph || '', parent:row, oninput:() =>
            {
                opt[key] = input.value.replace(/[^0-9]/g, '');
                input.value = opt[key];
            }});
        };

        numRow('fsGalaxy', 'Galaxy', 'current');
        numRow('fsSystem', 'System', 'current');
        numRow('fsPosition', 'Position', 'current');

        const missionRow = Util.addDom('label', { parent:container, 'data-label':'Mission' });
        const missionSel = Util.addDom('select', { parent:missionRow });
        [[4, 'Deployment'], [6, 'Espionage'], [7, 'Colonisation'], [8, 'Recycle debris']].forEach(m => Util.addDom('option', { parent:missionSel, value:m[0], child:m[1] }));
        missionSel.value = opt.fsMission || 4;
        missionSel.addEventListener('change', () => opt.fsMission = parseInt(missionSel.value));

        const speedRow = Util.addDom('label', { parent:container, 'data-label':'Speed' });
        const speedSel = Util.addDom('select', { parent:speedRow });
        for(let pct = 10; pct <= 100; pct += 10) Util.addDom('option', { parent:speedSel, value:pct, child:pct + '%' });
        speedSel.value = opt.fsSpeed || 10;
        speedSel.addEventListener('change', () => opt.fsSpeed = parseInt(speedSel.value));

        numRow('fsLeaveMetal', 'Leave metal', '0');
        numRow('fsLeaveCrystal', 'Leave crystal', '0');
        numRow('fsLeaveDeut', 'Leave deuterium', '0');
    ...
    addPinnedItemToList(player, list)
    {
        ...
        const rankLink = Util.addDom('a', { class:'ogl_ranking', href:`https://${window.location.host}/game/index.php?page=ingame&component=highscore&site=${page}&category=1&searchRelId=${player.uid}`, child:'#'+player.score?.globalRanking || '?' });
    ...
    openPinnedDetail(id, update)
    {
        ...
        const updateSide = () =>
        {
            ...
            {
                // v13 compat: #chatBar may be renamed
                if(!document.querySelector('#chatBar') && !document.querySelector('[id*="chatBar"]'))
                {
            ...
            {
                window.open(Util.genOstatsLink(this.ogl, id), '_blank');
            }});
    ...
    checkUpgrade()
    {
        ...
        // v13 compat: .planetlink/.moonlink may be renamed
        document.querySelectorAll('.planetlink, .moonlink, a[href*="cp="]').forEach(planet =>
        {
            if(!planet.getAttribute('href')?.includes('cp=')) return;
            ...
            const id = urlParams.get('cp')?.split('#')[0];
            if(!id) return;
            const isMoon = planet.classList.contains('moonlink') || (planet.closest('[class*="moon"]') !== null && planet.closest('.smallplanet') !== null && planet !== planet.closest('.smallplanet')?.querySelector('.planetlink'));
...
class FleetManager extends Manager
{
    load()
    {
        ...
        if(this.ogl.page == 'fleetdispatch')
        {
            ...
            // v13 compat: #fleet1 .content may be renamed
            const _fleet1Content = document.querySelector('#fleet1 .content') || document.querySelector('[id*="fleet1"] .content') || document.querySelector('[id*="fleet1"]');
            this.capacityWrapper = Util.addDom('div', { 'class':'capacityProgress', parent:_fleet1Content, onclick:() => document.querySelector(`.ogl_requiredShips .ogl_${this.ogl.db.options.defaultShip}`)?.click() });
    ...
    overwrite()
    {
        ...
        {
            // v13 compat: #planetList may be renamed
            const _pl = document.querySelector('#planetList') || document.querySelector('[id*="planetList"]');
            if(_pl) _pl.classList.add('ogl_notReady');
        });
        ...
        Util.overWrite('selectMission', fleetDispatcher, false, () =>
        {
            ...
            // v13 compat: #fleet2 may be renamed
            (document.querySelector('#fleet2') || document.querySelector('[id*="fleet2"]'))?.setAttribute('data-selected-mission', fleetDispatcher.mission);
        });
        ...
        Util.overWrite('refreshTargetPlanet', fleetDispatcher, () =>
        {
            ...
            if(fleetDispatcher.union)
            {
                ...
                if(acsEndTime)
                {
                    ...
                    // v13 compat: #fleetBriefingPart1 may be renamed
                    const _fbp1 = document.querySelector('#fleetBriefingPart1') || document.querySelector('[id*="fleetBriefing"]') || document.querySelector('[id*="fleet-briefing"]');
                    if(_fbp1) Util.addDom('hr', { prepend:_fbp1 });
                    const liLeft = Util.addDom('li', { class:'ogl_acsInfo', child:"Time remaining: ", prepend:_fbp1 });
                    const liOffset = Util.addDom('li', { class:'ogl_acsInfo', child:'Time difference: ', prepend:_fbp1 });
    ...
    init()
    {
        ...
        if(this.ogl.mode === 1 || this.ogl.mode === 2)
        {
            this.selectCargoShips();          // favourite cargo + optional fallback to cover all resources
            fleetDispatcher.selectMaxAll();
        }
        else if(this.ogl.mode === 7)          // PrOGect: sibling (ships) — all ships to the linked body except the limiter's
        {
            this.selectSiblingShips();
        }
        else if(this.ogl.mode === 5 && fleetDispatcher.expeditionCount < fleetDispatcher.maxExpeditionCount)
        {
            this.selectExpedition(this.ogl.db.lastExpeditionShip || this.ogl.db.options.defaultShip, this.ogl.db.lastExpeditionHoldTime);
        }
        else if(this.ogl.mode === 6)   // PrOGect: night fleet save (topbar button OR the 'N' shortcut)
        {
            // record the cycle origin ONCE so the moon-hop chain (redirectToPlanet) can stop when it
            // loops back to the starting body instead of cycling forever. §1.1: the send stays manual.
            const _u = new URL(window.location.href);
            if(!_u.searchParams.get('oglfirstsourceid'))
            {
                const _cur = this.ogl.account.planets.getCurrent();
                _u.searchParams.set('oglfirstsourceid', _cur.currentType == 'moon' ? _cur.moonID : _cur.id);
        ...
        if(this.ogl._dom.secondCol)
        {
            requestAnimationFrame(() =>
            {
                ...
                if(this.ogl._dom.sendAll) { this.ogl._dom.sendAll.classList.add('material-icons'); this.ogl._dom.sendAll.innerText = 'keyboard_double_arrow_right'; }
    ...
    setRealTarget(obj, forceParam)
    {
        ...
        // v13 compat: #galaxy/#system/#position may be renamed
        const _gqEl = document.querySelector('#galaxy') || document.querySelector('[name="galaxy"]') || document.querySelector('#galaxy_input');
        const _sqEl = document.querySelector('#system') || document.querySelector('[name="system"]') || document.querySelector('#system_input');
        const _pqEl = document.querySelector('#position') || document.querySelector('[name="position"]') || document.querySelector('#position_input');
        if(_gqEl) _gqEl.value = fleetDispatcher.realTarget.galaxy;
        if(_sqEl) _sqEl.value = fleetDispatcher.realTarget.system;
        if(_pqEl) _pqEl.value = fleetDispatcher.realTarget.position;
        ...
        {
            // v13 compat: .planet-koords may be renamed
            const _koordsElFD = planet.querySelector('.planet-koords') || planet.querySelector('[class*="koords"]') || planet.querySelector('[class*="coords"]');
            const coords = (_koordsElFD?.innerText || '0:0:0').split(':');
            ...
            {
                // v13 compat: planetlink/moonlink may be renamed
                const parent = fleetDispatcher.realTarget.type == 1
                    ? (planet.querySelector('.planetlink') || planet.querySelector('a[href*="cp="]:not([class*="moon"])'))
                    : fleetDispatcher.realTarget.type == 3
                        ? (planet.querySelector('.moonlink') || planet.querySelector('a[href*="cp="][class*="moon"]'))
                        : false;
    ...
    fleetSent(defaultRedirection)
    {
        ...
        if(this.ogl.mode === 1 || this.ogl.mode === 2 || this.ogl.mode === 6 || this.ogl.mode === 7 || (this.ogl.mode === 5 && this.ogl.db.options.expeditionRedirect))
        {
            //localStorage.setItem('syl-redirect', this.ogl.nextRedirection);
    ...
    // PrOGect: renders the limiter's "-X not selected" badge on a fleet resource. Shared by the
    // limiter (updateLimiter) and the night fleet-save (fleetSave) so the two never drift visually.
    // onClick opens the relevant editor (limiter popup / fleet-save preset).
    showResourceHeldBadge(resourceName, heldAmount, onClick)
    {
        ...
        techDom.querySelector('.ogl_maxShip')?.remove();
        const text = Util.addDom('div', { class:'ogl_maxShip', parent:techDom });
        text.innerHTML = `<b>-${Util.formatToUnits(heldAmount || 0, 0)}</b>`;
        if(onClick) text.addEventListener('click', onClick);
    ...
    favShip(shipID)
    {
        ...
        if(document.querySelector('.ogl_popup.ogl_active .ogl_config'))
        {
    ...
    // PrOGect: selects cargo ships with enough capacity to carry `amount` resources, preferring
    // the favourite cargo (options.defaultShip). If the favourite alone can't cover it, it falls back
    // to the other main cargo, then optionally to pathfinder/recycler, per options.cargoFallback:
    //   0 = off (favourite only)  1 = + other cargo  2 = + other cargo & pathfinder  3 = + other cargo & recycler
    // Only used by the resource transports (collect / sibling / todolist) - NOT shipsForResources'
    // other callers (expeditions, raids, ...). Does NOT load resources: the caller does that
    // (selectMaxAll or manual cargo*). Still one user-confirmed send (AGENTS.md §1.1).
    selectCargoShips(amount)
    {
        const opt = this.ogl.db.options;
        const pref = parseInt(opt.defaultShip);

        if(amount == null)
        {
            amount = 0;
            ['metal', 'crystal', 'deut', 'food'].forEach(r => amount += fleetDispatcher[this.resOnPlanet[r]] || 0);
        }

        // priority list: favourite cargo first, then the fallbacks enabled in the setting
        const order = [pref];
        if(opt.cargoFallback >= 1) [202, 203].forEach(c => { if(order.indexOf(c) < 0) order.push(c); }); // other main cargo
        if(opt.cargoFallback == 2 && order.indexOf(219) < 0) order.push(219);                            // pathfinder
        if(opt.cargoFallback == 3 && order.indexOf(209) < 0) order.push(209);                            // recycler
        ...
        order.forEach(sID =>
        {
            ...
            const available = fleetDispatcher.shipsOnPlanet.find(s => s.id == sID)?.number || 0;
            const cap = this.ogl.db.shipsCapacity[sID] || 0;
            if(available <= 0 || cap <= 0) return;
            const use = Math.min(Math.ceil(remaining / cap), available);
            fleetDispatcher.selectShip(sID, use);
            remaining -= use * cap;
        });
    }

    // PrOGect: sibling (ships) — select EVERY ship on the current body except the amounts the
    // limiter keeps here, to move the whole fleet in ONE user-confirmed send (AGENTS.md §1.1). The
    // target is NOT set here: it is the destination the player picked (carried by the URL), so this
    ...
    selectSiblingShips()
    {
        ...
        {
            const send = Math.max(0, ship.number - (keep[ship.id] || 0));
            if(send > 0) fleetDispatcher.selectShip(ship.id, send);
        });
    }

    selectExpedition(shipID, holdTime)
    {
        if(fleetDispatcher.fetchTargetPlayerDataTimeout) return;
        ...
        const _hMax = fleetDispatcher.maxHoldingTime || 99;
        const _ht = Math.max(1, Math.min(parseInt(holdTime) || 1, _hMax));
        fleetDispatcher.expeditionTime = _ht;
        // v13: also drive the visible dropdown, else updateExpeditionTime() re-reads the DOM (still 1) and resets it
        const _expTime = document.querySelector('#expeditiontime') || document.querySelector('[id*="expeditiontime"]');
        if(_expTime) _expTime.value = _ht;
        const _expTimeDd = document.querySelector('#expeditiontimeline .dropdown a') || document.querySelector('[id*="expeditiontimeline"] .dropdown a');
        if(_expTimeDd) _expTimeDd.innerText = _ht;
        fleetDispatcher.updateExpeditionTime();
        fleetDispatcher.refresh();
        fleetDispatcher.focusSubmitFleet1();

        this.ogl.db.lastExpeditionShip = shipID;
        this.ogl.db.lastExpeditionHoldTime = _ht;   // remember the hold time so the mode-5 redirect re-prefill (next planet) keeps it
    }

    // PrOGect: Fleet Save (night fleet). Fills the dispatch with the user's preset and focuses the
    // submit button — the PLAYER confirms every send with their own Enter (1 keystroke = 1 send,
    // AGENTS.md §1.1; never auto-submitted). After each send the moon-cycle (mode 6) navigates to the
    // NEXT moon and re-prefills here — exactly the expeditionRedirect pattern: only navigation +
    // pre-fill are automatic, the send stays manual. Resources to leave are shown on the fleet with
    // the SAME "-X not selected" badge as the limiter (showResourceHeldBadge).
    fleetSave()
    {
        if(fleetDispatcher.currentPage != 'fleet1') return;
        ...
        // destination: preset coords; any blank field falls back to the current planet's own coord
        const g = parseInt(opt.fsGalaxy);
        const s = parseInt(opt.fsSystem);
        const p = parseInt(opt.fsPosition);
        const dest = { galaxy:isNaN(g)?cur.galaxy:g, system:isNaN(s)?cur.system:s, position:isNaN(p)?cur.position:p };
        // abort if it resolves to THIS planet (no real destination set) — OGame rejects a send to self.
        if(dest.galaxy === cur.galaxy && dest.system === cur.system && dest.position === cur.position)
        {
            this.ogl._notification.addToQueue(this.ogl._lang.find('fleetSaveNoDest'), false);
            return false;
        }
        this.setRealTarget(fleetDispatcher.realTarget, { galaxy:dest.galaxy, system:dest.system, position:dest.position, type:(parseInt(opt.fsMission) === 8 ? 2 : 1), name:'' });

        fleetDispatcher.selectMission(parseInt(opt.fsMission) || 4);

        // resolve the speed from the REAL speed bar so we don't assume the internal scale (1-10 vs 10-100):
        // find the bar step whose label matches the chosen percent and use its data-step value verbatim.
        const pct = Math.max(10, Math.min(parseInt(opt.fsSpeed) || 10, 100));
        const _spd = document.querySelector('#speedPercentage') || document.querySelector('[id*="speedPercentage"]');
        let _stepVal = pct / 10; // fallback only if the bar isn't in the DOM
        if(_spd)
        {
            const _steps = Array.from(_spd.querySelectorAll('[data-step]'));
            const _hit = _steps.find(e => parseInt(e.textContent.replace(/\D/g, '')) === pct) || _steps[Math.round(pct / 10) - 1];
            if(_hit) _stepVal = _hit.getAttribute('data-step');
        }
        fleetDispatcher.speedPercent = _stepVal;
        fleetDispatcher.realSpeedPercent = _stepVal;
        fleetDispatcher.setFleetPercent(_stepVal);
        try { if(_spd) Object.values(_spd)[0]?.percentageBarInstance?.setValue(_stepVal); } catch(e) {}

        // Carry all resources minus the amounts to leave, and SHOW what's left behind with the SAME
        // limiter badge ("-X not selected"). Based on the INITIAL on-planet amounts so it doesn't
        // double-subtract if the limiter already reduced them. OGame clamps cargo to capacity.
        // Two things this has to respect - together they are the reported "the two limiters overlap":
        //  - the PROFILE limiter's reserve: both mechanisms write the same deuteriumOnPlanet / cargo*
        //    fields, so whichever ran last won, and a fleet save (fsLeave* default 0) silently discarded
        //    the limiter's keep and carried everything. Hold whichever of the two is larger instead.
        //  - the FUEL: it is charged to the planet's deuterium ON TOP of the deut loaded as cargo, so
        ...
        ['metal', 'crystal', 'deut'].forEach(resourceName =>
        {
            const onPlanet = (this.initialResOnPlanet && this.initialResOnPlanet[resourceName] != null) ? this.initialResOnPlanet[resourceName] : fleetDispatcher[this.resOnPlanet[resourceName]];
            const leave = Math.max(leaveMap[resourceName], limiterHold[resourceName] || 0);
    ...
    updateSpeedBar()
    {
        ...
        // v13 compat: #speedPercentage may be renamed
        const _speedPct = document.querySelector('#speedPercentage') || document.querySelector('[id*="speedPercentage"]') || document.querySelector('[id*="speed-percentage"]');
        if(!_speedPct) return;

        _speedPct.addEventListener('mousemove', event =>
        {
            _speedPct.querySelectorAll('.selected').forEach(e => e.classList.remove('selected'));
            _speedPct.querySelector(`[data-step="${fleetDispatcher.realSpeedPercent}"]`)?.classList.add('selected');
        });

        _speedPct.addEventListener('click', e =>
        {
            // fix OGame bug for ff mobile
            if(e.target.getAttribute('data-step'))
            {
                // v13 compat: #speedPercentage may be renamed (reuse _speedPct from closure)
                (_speedPct?.querySelector('.bar') || _speedPct)?.style && ((_speedPct.querySelector('.bar') || _speedPct).style.width = e.target.offsetLeft + e.target.offsetWidth + 'px');
                _speedPct?.querySelectorAll('.selected').forEach(e => e.classList.remove('selected'));
    ...
    prefillTodolistCargo()
    {
        ...
        this.selectCargoShips(Math.min(cumul, max));
    ...
    checkSendShips()
    {
        ...
        {
            // v13 compat: getAttribute('onclick') may be null
...
class GalaxyManager extends Manager
{
    init()
    {
        ...
        {
            this.isReady = true;
            if(typeof this.unloadSystem === 'function') this.unloadSystem();
            submitForm();
        }
    }
    ...
    load()
    {
        ...
        // v13 compat: galaxy/system global vars may not exist; fall back to URL params or inputs
        const _galaxyInput = document.querySelector('#galaxy_input') || document.querySelector('#galaxy');
        const _systemInput = document.querySelector('#system_input') || document.querySelector('#system');
        this.galaxy = unsafeWindow.galaxy ?? parseInt(_galaxyInput?.value) ?? parseInt(new URLSearchParams(window.location.search).get('galaxy')) ?? 1;
        this.system = unsafeWindow.system ?? parseInt(_systemInput?.value) ?? parseInt(new URLSearchParams(window.location.search).get('system')) ?? 1;

        if(!this.galaxy || !this.system) return; // still not available
        ...
        // v13 compat: #galaxyLoading may be renamed
        const loader = document.querySelector('#galaxyLoading') || document.querySelector('.galaxyLoading') || document.querySelector('[id*="galaxyLoading"]');
        if(loader) loader.setAttribute('data-currentposition', this.galaxy+':'+this.system);
        ...
        unsafeWindow.loadContentNew = function(g, s)
        {
            ...
            if(loader) loader.setAttribute('data-currentPosition', `${g}:${s}`);
            tippy.hideAll();
    ...
    check(data)
    {
        ...
        const _sysDataPreCheck = data && (data.system || data);
        // v13 compat: some servers omit data.success but still return valid sysData with galaxyContent/positions/content
        const _hasContentPreCheck = _sysDataPreCheck && (_sysDataPreCheck.galaxyContent || _sysDataPreCheck.content || _sysDataPreCheck.positions);
        if((!data.success && !_hasContentPreCheck) || !_sysDataPreCheck)
        {
            this.ogl._notification.addToQueue(`Error, cannot fetch [${this.galaxy}:${this.system}] data`);
            return;
        }

        // If rows not yet in DOM, wait one frame for OGame to render them
        const _proceedWithCheck = () =>
        {

        let isEmpty = true;

        // v13 compat: data.system may be nested differently; support both old and new API shapes
        const sysData = data.system || data;
        // v13 compat: some servers omit data.success but still return valid sysData with galaxyContent/positions/content
        const hasContent = sysData && (sysData.galaxyContent || sysData.content || sysData.positions);

        this.galaxy = sysData.galaxy;
        this.system = sysData.system;
        // v13: galaxy/system may be in numeric fields galaxy/system or galaxyId/systemId
        if(this.galaxy == null) this.galaxy = sysData.galaxyId ?? sysData.galaxy_id ?? this.galaxy;
        if(this.system == null) this.system = sysData.systemId ?? sysData.system_id ?? this.system;
        ...
        this.ogl.db.spyProbesCount = sysData.settingsProbeCount || 0;

        // v13 compat: helper to read resource amounts from {amount} or plain number
        const _resAmt = (res, key) =>
        {
            if(!res) return 0;
            const r = res[key] || res[key.charAt(0).toUpperCase() + key.slice(1)];
            if(r == null) return 0;
            return parseInt(typeof r === 'object' ? (r.amount ?? r.current ?? 0) : r) || 0;
        };

        // v13 compat: getActivity handles missing activity object
        let getActivity = element =>
        {
            if(!element.activity) return 60;
            if(element.activity.showActivity == 15) return '*'; // acti
            else return element.activity.idleTime || 60;
        }

        // v13 compat: galaxyContent may be at sysData.galaxyContent or sysData.content or sysData.positions
        const galaxyContent = sysData.galaxyContent || sysData.content || sysData.positions || [];

        galaxyContent.forEach(line =>
        {
            const position = line.position;
            const debris = { metal:0, crystal:0, deut:0, total:0 };
            const row = document.querySelector(`#galaxyRow${position}`) || document.querySelector(`[id*="galaxyRow${position}"]`);
            if(!row) return; // v13: skip if row not rendered

            if(position == 16)
            {
                // v13: p16 debris may be on line.planets.resources or line.resources
                const p16res = line.planets?.resources || line.resources || {};
                debris.metal = _resAmt(p16res, 'metal');
                debris.crystal = _resAmt(p16res, 'crystal');
                debris.deut = _resAmt(p16res, 'deuterium') || _resAmt(p16res, 'deut');
            ...
            // v13 compat: player info may be in line.player or line.playerData
            const playerData = line.player || line.playerData || {};
            const playerID = parseInt((playerData.playerId == 99999 || playerData.playerId == null) ? -1 : playerData.playerId);
            ...
            //const playerStatusTag = playerData.isOnVacation ? 'v' : playerData.isLongInactive ? 'I' : playerData.isInactive ? 'i' : 'n';
            const isOwn = playerID == this.ogl.account.id;
            const rank = playerData.highscorePositionPlayer ?? playerData.rank ?? -1;
            const activities = [];
            ...
            // v13: planets array may be at line.planets (array) or line.bodies or line.slots
            const planetsArray = Array.isArray(line.planets) ? line.planets : (line.bodies || line.slots || []);
            ...
            planetsArray.forEach(element =>
            {
                if(element.planetType == 1) // planet
                {
                    ...
                    if(!playerData.isInactive) isEmpty = false;
                }
                else if(element.planetType == 2) // debris
                {
                    const debRes = element.resources || {};
                    debris.metal = _resAmt(debRes, 'metal');
                    debris.crystal = _resAmt(debRes, 'crystal');
                    debris.deut = _resAmt(debRes, 'deuterium') || _resAmt(debRes, 'deut');
                    debris.total = debris.metal + debris.crystal + debris.deut;
        ...
        // Vue renders galaxy rows asynchronously - use MutationObserver to wait for them
        ...
        {
            // Observe galaxy container for Vue to inject rows
    ...
    unloadSystem()
    {
        // v13 compat: #galaxyRow16 may be renamed
        const _row16 = document.querySelector('#galaxyRow16') || document.querySelector('[id*="galaxyRow16"]');
        if(_row16)
        {
            document.querySelector('.ogl_expeditionRow') && document.querySelector('.ogl_expeditionRow').remove();
            _row16.classList.remove('ogl_important');
        }
    ...
    checkCurrentSystem()
    {
        ...
        // v13 compat: #galaxyContent may be renamed
        if(this.ogl._tooltip) this.ogl._tooltip.initTooltipList(document.querySelectorAll('#galaxyContent .tooltipClick, #galaxyContent .ownPlayerRow, [id*="galaxyContent"] .tooltipClick, [id*="galaxyContent"] .ownPlayerRow, .ogl_moreGalaxyInfo .tooltip'));
        //initTooltips();
...
class JumpgateManager extends Manager
{
    ...
    check()
    {
        // v13 compat: #jumpgate may be renamed
        const parent = document.querySelector('#jumpgate') || document.querySelector('[id*="jumpgate"]') || document.querySelector('[class*="jumpgate"]');
        ...
        // v13 compat: #jumpgateForm may be renamed
        const _jgForm = document.querySelector('#jumpgateForm') || document.querySelector('[id*="jumpgateForm"]') || document.querySelector('form[name*="jumpgate"]');
        const limiterField = Util.addDom('fieldset', { parent:(_jgForm?.querySelector('.ship_selection_table') || _jgForm) });
    ...
    updateLimiter()
    {
        // v13 compat: #jumpgate/#jumpgateNotReady may be renamed
        const _jg = document.querySelector('#jumpgate') || document.querySelector('[id*="jumpgate"]');
        if(!_jg || document.querySelector('#jumpgateNotReady') || document.querySelector('[id*="jumpgateNotReady"]')) return;
        ...
        document.querySelectorAll('#jumpgate .ship_input_row').forEach(line =>
        {
            ...
            // v13 compat: .quantity/.a may be absent
        ...
        // v13 compat: #jumpgate #sendall may be renamed
        const _sendAllBtn = document.querySelector('#jumpgate #sendall') || document.querySelector('[id*="jumpgate"] [id*="sendall"]') || document.querySelector('[id*="jumpgate"] button[type="submit"]');
        if(_sendAllBtn) _sendAllBtn.setAttribute('onclick', `setMaxIntInput("#jumpgateForm", ${JSON.stringify(sendAllJson)})`);
    }

    checkPhalanx()
    {
        // v13 compat: .phalanx dialog title may have different structure
        const _phalanxEl = document.querySelector('.ui-dialog .phalanx') || document.querySelector('[class*="ui-dialog"] [class*="phalanx"]');
        const parent = _phalanxEl?.parentElement?.querySelector('.ui-dialog-titlebar .ui-dialog-title') || _phalanxEl?.closest('[class*="ui-dialog"]')?.querySelector('[class*="title"]');
    ...
    saveTimer()
    {
        ...
        jumpgateDone = a =>
        {
            ...
            if(a.status)
            {
                ...
                // v13 compat: planet entry may be absent
                const destinationLevel = this.ogl._dom.planet?.[jumpGateTargetId]?.getAttribute?.('data-jumpgatelevel') || 0;
...
class AccountManager extends Manager
{
    ...
    openProduction()
    {
        ...
        // FetchManager.fetchEmpireFields) - prime it here too so the popup self-heals within a
        // couple opens even if the player never visits the empire page directly.
        this.ogl._fetch.fetchEmpireFields(0);
        this.ogl._fetch.fetchEmpireFields(1);
        ...
        // v13 compat: .planetlink may be renamed. Exclude moon links: a moon anchor also
        // matches a[href*="cp="], so without the exclusion each system produced two rows here
        // (the real planet row, plus a spurious "undefined" row for the moon's own cp= link).
        document.querySelectorAll('.smallplanet .planetlink, .smallplanet a[href*="cp="]:not([class*="moon"])').forEach(item =>
        {
            if(!item.getAttribute('href')?.includes('cp=')) return;
            ...
            `
                ...
                <a href="${item.getAttribute('href')}"><img src="${(item.querySelector('.planetPic') || item.querySelector('.planetlink img') || item.querySelector('a[href*="cp="] img'))?.getAttribute('src') || ''}"></a>
                ${
                    item._ogl.moonLink
                    ? `<a href="${item._ogl.moonLink.getAttribute('href')}"><img src="${(item._ogl.moonLink.querySelector('img'))?.getAttribute('src') || ''}"></a>`
                    : '<div>-</div>'
                }
    ...
    openPlanets(type, tech)
    {
        ...
        // v13 compat: .planetlink/.moonlink may be renamed
        const selector = type == 'planets' ? '.smallplanet .planetlink, .smallplanet a[href*="cp="]:not([class*="moon"])' : '.smallplanet .moonlink, .smallplanet a[href*="cp="][class*="moon"]';
...
class TooltipManager extends Manager
{
    load()
    {
        ...
        // v13 compat: #metal_box may be renamed
        if(!(document.querySelector('#metal_box') || document.querySelector('[id*="metal_box"]') || document.querySelector('[id*="metal-box"]'))?.getAttribute('title')) getAjaxResourcebox();
                ...
                if(element._tippy)
                {
                    element._tippy.setProps(
                    {
                        ...
                        onShow:instance =>
                        {
                            ...
                            // v13 compat: planetlink/moonlink may be renamed
                            if(element.classList.contains('planetlink') || (element.closest('.smallplanet') && element.closest('a[href*="cp="]') && !element.classList.contains('moonlink'))) content = this.updatePlanetMenuTooltip(instance, 'planet') ?? content;
                            else if(element.classList.contains('moonlink') || element.closest('.smallplanet')?.querySelector('.moonlink') === element) content = this.updatePlanetMenuTooltip(instance, 'moon') ?? content;
    ...
    updatePlanetMenuTooltip(instance, refType)
    {
        ...
        if(refType == 'planet')
        {
            ...
            // v13 compat: parent id may be 'planet-N' or just the id number
            const _parentId = instance.reference.parentNode.getAttribute('id') || '';
            const planetID = _parentId.replace('planet-', '') || '';
            // v13 compat: .planet-name may be renamed
            const _pNameEl = instance.reference.querySelector('.planet-name') || instance.reference.querySelector('[class*="planet-name"]') || instance.reference.querySelector('.planetname') || instance.reference.querySelector('[class*="planetname"]');
            const name = _pNameEl?.innerText || '';
            ...
            `
                <div>
                    ...
                    <div>
                        ...
                        <div>${(instance.reference.querySelector('.planetPic') || instance.reference.querySelector('.planetlink img') || instance.reference.querySelector('a[href*="cp="] img'))?.outerHTML || ''}</div>
                    </div>
        ...
        else if(refType == 'moon')
        {
            ...
            // v13 compat: href/cp may differ; icon-moon may be renamed
            const urlParams = new URLSearchParams(instance.reference.getAttribute('href') || '');
            const planetID = urlParams.get('cp')?.split('#')[0] || '';
            const _iconMoon = instance.reference.querySelector('.icon-moon') || instance.reference.querySelector('[class*="moon"] img') || instance.reference.querySelector('img');
            const name = _iconMoon?.getAttribute('alt') || _iconMoon?.getAttribute('title') || '';
            const data = this.ogl.db.myPlanets[planetID];
            const div = Util.addDom('div', { class:'ogl_planetTooltip' });
            const links = new DOMParser().parseFromString(instance.props.content, 'text/html').querySelectorAll('a');
            ...
            `
                <div>
                    ...
                    <div>
                        ...
                        <div>${(instance.reference.querySelector('.icon-moon') || instance.reference.querySelector('[class*="moon"] img') || instance.reference.querySelector('img'))?.outerHTML || ''}</div>
                    </div>
                    <div class="ogl_planetTooltipData">
                        <div class="ogl_textCenter"><i class="material-icons">flex_wrap</i>${data.fieldUsed}/${data.fieldMax}</div>
    ...
    updateFleetTooltip(instance)
    {
        ...
        // v13 compat: coordsOrigin/destCoords selectors may be renamed
        let origin = (sender.closest('.eventFleet')?.querySelector('.coordsOrigin, .originCoords, [class*="coordsOrigin"]') || sender.closest('.fleetDetails')?.querySelector('.originData a, [class*="originData"] a'))?.innerText.slice(1, -1);
        origin = origin || { galaxy:0, system:0, position:0 };
        let originType = (sender.closest('.eventFleet')?.querySelector('.originFleet figure.moon, .originFleet .moon, [class*="originFleet"] .moon') || sender.closest('.fleetDetails')?.querySelector('.originPlanet figure.moon, [class*="originPlanet"] .moon')) ? 'moon' : 'planet';

        let destination = (sender.closest('.eventFleet')?.querySelector('.destCoords, .destinationCoords, [class*="destCoords"]') || sender.closest('.fleetDetails')?.querySelector('.destinationData a, [class*="destinationData"] a'))?.innerText.slice(1, -1);
        destination = destination || { galaxy:0, system:0, position:0 };
        let destinationType = (sender.closest('.eventFleet')?.querySelector('.destFleet figure.moon, .destFleet .moon, [class*="destFleet"] .moon') || sender.closest('.fleetDetails')?.querySelector('.destinationPlanet figure.moon, [class*="destinationPlanet"] .moon')) ? 'moon' : 'planet';

        // v13 compat: .planet-koords may be renamed
        const _planetKoordsSel = '.planet-koords, [class*="koords"], [class*="planet-coords"]';
        const meAsAttacker = Array.from(document.querySelectorAll(_planetKoordsSel)).find(e => e.innerText == origin) ? true : false;
        const meAsDefender = Array.from(document.querySelectorAll(_planetKoordsSel)).find(e => e.innerText == destination) ? true : false;
        ...
        xml.querySelectorAll('tr').forEach(line =>
        {
            ...
            if(key && value)
            {
                ...
                const val = Util.formatNumber(parseInt(value)); // ships + resources shown in full (no k/M rounding) per user request
...
class MessageManager extends Manager
{
    load()
    {
        ...
        // v13 compat: ogame.messages may be undefined or have renamed paginator methods
        if(unsafeWindow.ogame?.messages)
        {
            const _om = unsafeWindow.ogame.messages;
            if(_om.paginatorPrevious) Util.overWrite('paginatorPrevious', _om, false, () => { if(this.tabID != 20) this.checkTab() });
            if(_om.paginatorNext) Util.overWrite('paginatorNext', _om, false, () => { if(this.tabID != 20) this.checkTab() });
            if(_om.paginatorFirst) Util.overWrite('paginatorFirst', _om, false, () => { if(this.tabID != 20) this.checkTab() });
            if(_om.paginatorLast) Util.overWrite('paginatorLast', _om, false, () => { if(this.tabID != 20) this.checkTab() });
        }
    ...
    readMessagesData()
    {
        ...
        const messageIds = [];
        // v13 compat: ogame.messages.content may be an array or different API
        const _msgContent = ogame?.messages?.content;
        const _msgHtml = Array.isArray(_msgContent) ? _msgContent.join('') : (typeof _msgContent === 'string' ? _msgContent : document.querySelector('#messagewrapper, .messages-content, #content')?.innerHTML || '');
        const xml = new DOMParser().parseFromString(_msgHtml, 'text/html');
        xml.querySelectorAll('.msg').forEach(xmlLine =>
        {
            ...
            // v13 compat: tab IDs may have shifted; also match globalTypeID as fallback
            const _isSpyReport = message.globalTypeID == 21 || message.globalTypeID == 7; // 21=spy, 7=probe
            const _isCombat = message.globalTypeID == 25;
            const _isExpedition = message.globalTypeID == 29;
            const _isProbe = message.globalTypeID == 48;

            if((this.tabID == 20 || this.tabID == 25 || _isSpyReport) && this.ogl.db.options.displaySpyTable) // spies
            {
                this.readSpyData(message, xmlLine, rawData, dom);
            }
            else if(this.tabID == 21 || _isCombat || _isProbe) // combat
            {
                if(_isCombat || message.globalTypeID == 25) this.readCombatData(message, xmlLine, rawData, dom);
                else if(_isProbe || message.globalTypeID == 48) this.readProbeData(message, xmlLine, rawData, dom);
            }
            else if(this.tabID == 22 || _isExpedition) // expeditions & discoveries
            {
    ...
    loadSpyTable(isOutdated, forceClean)
    {
        ...
        const currentPlanet = this.ogl.currentPlanet?.obj?.coords ? (([g, s, p]) => ({ galaxy: g, system: s, position: p }))(this.ogl.currentPlanet.obj.coords.split(':').map(Number)) : { galaxy: 0, system: 0, position: 0 };
    ...
    // PrOGect: renders ONE rule set (type = 'ignore' | 'highlight') into `section`, re-rendering
    // in place on add/delete so it works both as a standalone popup and inline inside the settings.
    renderSpyRuleSet(section, type)
    {
        section.innerText = '';
        ...
        const title = type == 'ignore' ? '<i class="material-icons ogl_danger">block</i> Ignore all reports with...' : '<i class="material-icons ogl_caution">star</i> Highlight all reports with...';

        const rulePicker = Util.addDom('fieldset', { parent:section, class:'ogl_addRule' });
        const rulesList = Util.addDom('div', { parent:section, class:'ogl_rulesList' });
        ...
        Util.addDom('button', { parent:rulePicker, child:'<i>+</i> add a rule', class:'ogl_button', onclick:() =>
        {
            ...
            this.renderSpyRuleSet(section, type);
            this.loadSpyTable();
        }});
    ...
    // standalone popup (from the spy table page action) - single rule set for the clicked type
    openSpyTableRules(type)
    {
        const container = Util.addDom('div', { class:'ogl_spyTableRules' });
        this.renderSpyRuleSet(container, type || 'ignore');
        this.ogl._popup.open(container);
    }

    // PrOGect: settings host - both rule sets (ignore + highlight) in one container for openDetail.
    buildSpyTableRules()
    {
        const container = Util.addDom('div', { class:'ogl_spyTableRules' });
        ['ignore', 'highlight'].forEach(type =>
        {
            const section = Util.addDom('div', { parent:container, class:'ogl_spyRuleSet' });
            this.renderSpyRuleSet(section, type);
        });
        return container;
    }
    ...
    summarize(message)
    {
        ...
        // PrOGect (ACS): on a recycle report from a battle debris field (type 32, pos 1-15) let the
        // player enter their OWN share of the recycled resources. In ACS the field is split but the report
        // shows everything one recycler hauled; the % rescales ONLY this report's contribution to the
        // Attacks stats (default 100% = no change). Manual, per-report — no auto-registration (AGENTS.md).
        const _dpos = (message.coords || '0:0:0').split(':')[2];
        if(message.globalTypeID == 32 && _dpos != 16 && message.id && message.date)
        {
            const day = this.ogl._stats.getDayStats(this.ogl._time.timeToKey(message.date.server));
            day.debris = day.debris || {};
            day.debris.reports = day.debris.reports || {};
            day.debris.shares = day.debris.shares || {};
            const curShare = day.debris.shares[message.id] == null ? 1 : day.debris.shares[message.id];

            const share = Util.addDom('div', { class:'ogl_acsShare tooltip' + (curShare < 1 ? ' ogl_acsShareActive' : ''), title:'ACS — your share of this recycled debris (counted in Attacks stats)', parent:recap });
            Util.addDom('i', { class:'material-icons', child:'group', parent:share });
            const input = Util.addDom('input', { type:'text', value:Math.round(curShare * 100), parent:share });
            Util.addDom('span', { child:'%', parent:share });

            share.addEventListener('click', e => e.stopPropagation()); // don't toggle the report body

            input.addEventListener('change', () =>
            {
                let pct = parseInt((input.value || '').replace(/\D/g, ''));
                if(isNaN(pct)) pct = 100;
                pct = Math.max(0, Math.min(100, pct));
                input.value = pct;
                const newShare = pct / 100;

                // ensure the raw amount is known (covers reports statified before this feature / prior sessions)
                if(!day.debris.reports[message.id])
                {
                    const raw = {};
                    for(const [k, v] of Object.entries(message.gain)) if(!isNaN(v) && v != 0 && (k === 'metal' || k === 'crystal' || k === 'deut')) raw[k] = v;
                    day.debris.reports[message.id] = raw;
                }

                const oldShare = day.debris.shares[message.id] == null ? 1 : day.debris.shares[message.id];
                const raw = day.debris.reports[message.id] || {};
                day.debris.gain = day.debris.gain || {};
                // incremental delta keeps historical debris intact (no full recompute needed)
                for(const [res, amt] of Object.entries(raw)) day.debris.gain[res] = (day.debris.gain[res] || 0) + amt * (newShare - oldShare);
                day.debris.shares[message.id] = newShare;

                share.classList.toggle('ogl_acsShareActive', newShare < 1);
                this.ogl.save();
                this.ogl._stats.showMiniRecap();
            });
        }
    ...
    statify(message)
    {
        ...
        if(type)
        {
            ...
            // PrOGect (ACS): remember this recycle report's raw gain per message id so a manually
            // entered "my share" can later rescale ONLY this report's contribution (see summarize).
            // Baseline share = 1 — the loop above already counted the full recycled amount.
            if(type === 'debris' && message.id)
            {
                stats.debris.reports = stats.debris.reports || {};
                stats.debris.shares = stats.debris.shares || {};
                const raw = {};
                for(const [gid, amt] of Object.entries(message.gain)) if(!isNaN(amt) && amt != 0 && (gid === 'metal' || gid === 'crystal' || gid === 'deut')) raw[gid] = amt;
                stats.debris.reports[message.id] = raw;
                if(stats.debris.shares[message.id] == null) stats.debris.shares[message.id] = 1;
            }

            // v13 compat: #ogame-tracker-menu may be absent or renamed
            if(!document.querySelector('#ogame-tracker-menu') && !document.querySelector('[id*="ogame-tracker"]')) this.ogl._notification.addToQueue(false, undefined, notificationData);
        }
    }
    ...
    addBoardTab()
    {
        ...
        {
            // v13 compat: #messagewrapper may be renamed
            const wrapper = document.querySelector('#messagewrapper') || document.querySelector('[id*="messagewrapper"]') || document.querySelector('[id*="message-wrapper"]');
            if(!wrapper) return;
            wrapper.innerText = '';
...
class MovementManager extends Manager
{
    load(reload)
    {
        ...
        // v13: #eventboxContent exists but is empty until fetched (v12 pre-rendered .eventFleet rows;
        // v13 does not), so gate on the box existing and fetch the event list once on this page load
        // (a single background call on page load — allowed by AGENTS.md §4; not a timer/poll). The
        // response is processed directly in the success callback below, because the global jQuery
        // ajaxSuccess hook does not reliably fire for this request on v13.
        if(document.querySelector('#eventboxContent'))
        {
            // eventlistLink may be undefined in new OGame versions
            const _evtLink = (typeof eventlistLink !== 'undefined' && eventlistLink)
                || `https://${window.location.host}/game/index.php?page=componentOnly&component=eventList&action=fetchEventBox&ajax=1&asJson=1`;
            $.ajax({
                url:_evtLink,
                success:(data, status, xhr) =>
                {
                    let raw = (typeof data === 'string') ? data : ((xhr && xhr.responseText) || '');
                    let html = raw;
                    // v13 returns JSON { content: { eventlist } }; older OGame used { components: { eventList } }
                    try { const j = JSON.parse(raw); html = j?.content?.eventlist || j?.components?.eventList || (typeof j === 'string' ? j : raw); } catch(e) {}
                    this.check(new DOMParser().parseFromString(html, 'text/html'));
        ...
        document.querySelectorAll('.route a').forEach((route, index) =>
        {
            ...
            htmlData.querySelectorAll('.fleetinfo tr').forEach(subline =>
            {
                if(subline.querySelector('td') && subline.querySelector('.value'))
                {
                    ...
                    if(!isNaN(key))
                    {
                        Util.addDom('div', { class:`ogl_icon ogl_${key}`, child:Util.formatNumber(value), parent:resourcesBlock }); // ships shown in full (no k/M rounding) per user request
                    }
                    else
                    {
    ...
    check(xml)
    {
        ...
        xml.querySelectorAll('#eventContent tbody tr').forEach(line =>
        {
            ...
            // v13 compat: coordsOrigin/destCoords may be renamed; use robust fallback
            const _getCoordEl = (el, ...selectors) =>
            {
                for(const sel of selectors) { const r = el.querySelector(sel); if(r) return r; }
                return null;
            };
            // v13 fix: `line` comes from a detached DOMParser document (never attached to the
            // window), so .innerText always returns "" there (it requires a render tree).
            // .textContent works on detached nodes and gives the same result here.
            const _getCoordText = el => el ? el.textContent.trim().replace(/[\[\]]/g, '') : '';

            const _coordsOriginEl = _getCoordEl(line, '.coordsOrigin', '.originCoords', '.originData a', '[class*="coordsOrigin"]', '[class*="origin"] a');
            const _destCoordsEl = _getCoordEl(line, '.destCoords', '.destinationCoords', '.destinationData a', '[class*="destCoords"]', '[class*="destination"] a');

            const _koordsSel = '#planetList .planet-koords, #planetList [class*="koords"], #planetList [class*="coords"]';

            movement.from = {};
            const _fromCoords = _getCoordText(_coordsOriginEl);
            movement.from.anotherPlayer = !Boolean(Array.from(document.querySelectorAll(_koordsSel)).find(p => p.innerText.trim().replace(/[\[\]]/g, '') === _fromCoords));
            movement.from.isMoon = Boolean(line.querySelector('.originFleet figure.moon, .originFleet .moon, [class*="originFleet"] .moon'));
            movement.from.coords = _fromCoords;

            movement.to = {};
            const _toCoords = _getCoordText(_destCoordsEl);
            movement.to.anotherPlayer = !Boolean(Array.from(document.querySelectorAll(_koordsSel)).find(p => p.innerText.trim().replace(/[\[\]]/g, '') === _toCoords));
            movement.to.isMoon = Boolean(line.querySelector('.destFleet figure.moon, .destFleet .moon, [class*="destFleet"] .moon'));
            movement.to.coords = _toCoords;
            ...
            {
                // v13 compat: destCoords/planet-koords may be renamed; use movement.to.coords from above
                const dest = Array.from(document.querySelectorAll('#planetList .planet-koords, #planetList [class*="koords"], #planetList [class*="coords"]')).find(p => p.innerText.trim().replace(/[\[\]]/g, '') === movement.to.coords);
                
                if(dest)
                {
                    const destSmallPlanet = dest.closest('.smallplanet');
                    // v13 compat: moonlink/planetlink may be renamed
                    const destTarget = movement.to.isMoon
                        ? (destSmallPlanet.querySelector('.moonlink') || destSmallPlanet.querySelector('a[href*="cp="][class*="moon"]'))
                        : (destSmallPlanet.querySelector('.planetlink') || destSmallPlanet.querySelector('a[href*="cp="]'));
            ...
            tooltip.querySelectorAll('.fleetinfo tr').forEach(subline =>
            {
                if(subline.querySelector('td') && subline.querySelector('.value'))
                {
                    ...
                    let value = subline.querySelector('.value').textContent.replace(/\.|,| /g, '');
...
class ShortcutManager extends Manager
{
    load()
    {
        ...
        // v13 compat: #rechts may be renamed
        const _rechts = document.querySelector('#rechts') || document.querySelector('[id*="rechts"]') || document.querySelector('[id*="right-column"]') || document.body;
        this.shortcutDiv = Util.addDom('div', { class:'ogl_shortcuts', parent:this.ogl.db.options.shortcutsOnRight ? _rechts : this.shortCutWrapper });
        this.locked = false;
        ...
        this.add('showMenuResources', () =>
        {
            ...
            localStorage.setItem('syl_menulayout', this.ogl.db.options.showMenuResources);
        ...
        if(this.ogl.page == 'fleetdispatch')
        {
            ...
            // PrOGect: night expedition — one expedition with the preset hold time (default 8h). 1 tasto = 1 invio.
            this.add('expeditionNight', () => { if(fleetDispatcher.currentPage == 'fleet1') this.ogl._fleet.selectExpedition(this.ogl.db.lastExpeditionShip || this.ogl.db.options.defaultShip, this.ogl.db.options.expeditionHoldTime || 8); }, 'fleet');
            // PrOGect: night fleet save — fills the preset, player confirms the send. 1 click = 1 invio.
            this.add('fleetSave', () =>
            {
                if(fleetDispatcher.currentPage != 'fleet1') return;
                // PrOGect: arm the night-fleet moon cycle. Mark mode 6 + record the origin body so
                // the post-send redirect can hop to the next moon and stop when it loops back. The SEND
                // is always the player's own Enter (1 keystroke = 1 send, never auto-submitted) - §1.1.
                this.ogl.mode = 6;
                const u = new URL(window.location.href);
                if(!u.searchParams.get('oglfirstsourceid'))
                {
                    const cur = this.ogl.account.planets.getCurrent();
                    u.searchParams.set('oglmode', '6');
                    u.searchParams.set('oglfirstsourceid', cur.currentType == 'moon' ? cur.moonID : cur.id);
                    history.replaceState(null, '', u.toString());
                }
                return this.ogl._fleet.fleetSave();
            }, 'fleet');
            ...
            this.add('fleetRepeat', () =>
            {
                ...
                {
                    // v13 compat: #fleet2 #expeditiontime may be renamed
                    const _expTime = document.querySelector('#fleet2 #expeditiontime') || document.querySelector('[id*="fleet2"] [id*="expeditiontime"]') || document.querySelector('#expeditiontime');
                    if(_expTime) _expTime.value = this.ogl.db.previousFleet.expeditionTime;
                    const _expTimeDd = document.querySelector('#fleet2 #expeditiontimeline .dropdown a') || document.querySelector('[id*="fleet2"] [id*="expeditiontimeline"] .dropdown a') || document.querySelector('#expeditiontimeline .dropdown a');
                    if(_expTimeDd) _expTimeDd.innerText = this.ogl.db.previousFleet.expeditionTime;
                    fleetDispatcher.updateExpeditionTime();
                }

                fleetDispatcher.setFleetPercent(fleetDispatcher.realSpeedPercent);

                // v13 compat: #speedPercentage may be renamed; percentageBarInstance may be absent
                try
                {
                    const _spd = document.querySelector('#speedPercentage') || document.querySelector('[id*="speedPercentage"]');
                    if(_spd) Object.values(_spd)[0]?.percentageBarInstance?.setValue(fleetDispatcher.realSpeedPercent);
                }
                catch(e) {}
            ...
            this.add('fleetResourcesSplit', keyNumber =>
            {
                ...
                else if(fleetDispatcher.currentPage == 'fleet2')
                {
                    ...
                    {
                        // v13 compat: #fleet2 #resources may be renamed
                        document.querySelectorAll('#fleet2 #resources .res_wrap, [id*="fleet2"] #resources .res_wrap, [id*="fleet2"] [id*="resources"] .res_wrap').forEach((resource, index) =>
                        {
            ...
            {
                if(!this.ogl._fleet.isReady) return false;
                // v13 compat: #fleetdispatchcomponent may be renamed
                (document.querySelector('#fleetdispatchcomponent') || document.querySelector('[id*="fleetdispatch"]'))?.dispatchEvent(new KeyboardEvent('keypress', { keyCode:13 }));
            }});
        }
        ...
        else if(this.ogl.page == 'movement')
        {
            ...
            {
                // v13 compat: #movementcomponent may be renamed
                (document.querySelector('#movementcomponent .reversal a[data-key-color="orange"]') || document.querySelector('[id*="movementcomponent"] .reversal a[data-key-color="orange"]') || document.querySelector('[id*="movement"] .reversal a[data-key-color="orange"]'))?.click();
            }, false, 'orange');

            this.add('backLastFleet', () =>
            {
                // v13 compat: #movementcomponent may be renamed
                (document.querySelector('#movementcomponent .reversal a[data-key-color="violet"]') || document.querySelector('[id*="movementcomponent"] .reversal a[data-key-color="violet"]') || document.querySelector('[id*="movement"] .reversal a[data-key-color="violet"]'))?.click();
            }, false, 'violet');
        }
...
class StatsManager extends Manager
{
    ...
    showDetails()
    {
        ...
        this.activeStatsTab = this.activeStatsTab || 'expe';
        const isRaid = this.activeStatsTab === 'raid';
        // expeditions tab = expedition missions + their pos-16 debris; attacks tab = combat loot +
        // recycled attack debris (pos 1-15). Keeping them apart is the point of the split.
        const tabKeys = isRaid ? ['raid', 'debris'] : ['expe', 'debris16'];

        // In-place re-render: when the popup is already open (tab switch / range change) we swap the
        // inner sections instead of rebuilding + reopening the whole popup — no reload flash, stable size.
        const popupActive = this.ogl._popup.popup?.classList.contains('ogl_active');
        const dom = this._statsDom;
        const reuse = dom && popupActive && this.ogl._popup.popup.contains(dom.container);

        let container, tabs, month, dateBar, details;
        if(reuse)
        {
            ({ container, tabs, month, details } = dom);
            container.classList.toggle('ogl_statsRaidTab', isRaid);
            tabs.innerText = ''; month.innerText = ''; details.innerText = '';
            // fresh dateBar node so initRangeSelection's listeners don't stack across renders
            dateBar = Util.addDom('div', { class:'ogl_dateBar' });
            dom.dateBar.replaceWith(dateBar);
            dom.dateBar = dateBar;
        }
        else
        {
            container = Util.addDom('div', { class:'ogl_stats' + (isRaid ? ' ogl_statsRaidTab' : '') });
            tabs = Util.addDom('div', { class:'ogl_statsTabs', parent:container });
            month = Util.addDom('div', { parent:container, class:'ogl_statsMonth' });
            dateBar = Util.addDom('div', { parent:container, class:'ogl_dateBar' });
            details = Util.addDom('div', { parent:container, class:'ogl_statsDetails' });
            this._statsDom = { container, tabs, month, dateBar, details };
        }

        // two-tab switcher (expeditions / attacks) — clicking re-runs showDetails, which now re-renders in place
        const makeTab = (id, label, icon) =>
        {
            const tab = Util.addDom('div', { class:'ogl_statsTab' + (this.activeStatsTab === id ? ' ogl_active' : ''), parent:tabs, child:`<i class="material-icons">${icon}</i><span>${label}</span>` });
            tab.addEventListener('click', () => { this.activeStatsTab = id; this.showDetails(); });
        };
        makeTab('expe', 'Expeditions', 'compass');
        makeTab('raid', 'Attacks', 'swords');
        ...
        // day bars — computed from the ACTIVE tab's types only, so the graph reflects one activity
        ...
        if(!reuse) this.ogl._popup.open(container, true);
    }
    ...
    initRangeSelection(bars, lowest, highest)
    {
        ...
        const maxAbs = Math.max(Math.abs(highest), Math.abs(lowest)) || 1;

        bars.querySelectorAll('.ogl_item').forEach(bar =>
        {
            const value = parseInt(bar.getAttribute('data-value'));
            // diverging baseline: bar height is a share of the half-track (max 30px), anchored to the
            // zero line at the item's vertical center; positive grows up, negative grows down.
            const height = value == 0 ? 0 : Math.min(30, Math.max(4, Math.round(Math.abs(value) / maxAbs * 30)));

            const content = Util.addDom('div', { parent:bar, class:`ogl_dbar ${value >= 0 ? 'ogl_pos' : 'ogl_neg'}` });
    ...
    updateShips(container, data)
    {
        ...
        // ships only make sense on the expeditions tab (finds); the attacks tab hides the grid entirely
        if(this.activeStatsTab === 'raid') return;

        const key = 'expe';

        [202,203,204,205,206,207,208,209,210,211,213,214,215,218,219].forEach(shipId =>
        {
            const value = (data?.[key]?.gain?.[shipId] || 0) - (data?.[key]?.gain?.[-shipId] || 0);

            Util.addDom('div', { class:`ogl_icon ogl_${shipId} tooltip${value ? '' : ' ogl_off'}`, title:`<div class="ogl_infoStats"><h3 class="ogl_textCenter">${this.ogl._lang.find(shipId)}</h3><hr><b>${Util.formatNumber(value)}</b></div>`, parent:container, child:Util.formatToUnits(value || '-', false, true) });
        });
    }
    ...
    {
        container.innerText = '';
        container.removeAttribute('data-pie'); // drop the old donut centre-label hook

        // outcome breakdown is expedition-only; the attacks tab has no such data
        if(this.activeStatsTab === 'raid') return;
        ...
        const colors =
        {
            resource:'#86edfd', darkmatter:'#b58cdb', ship:'#1dd1a1', nothing:'#c9d3dd',
            battle:'#ffd60b', trader:'#ff7d30', item:'#bf6c4d', duration:'#df5252', blackhole:'#818181',
        };

        const pieData =
        {
            resource: occurences.resource || 0,
            darkmatter: occurences.darkmatter || 0,
            ship: occurences.ship || 0,
            nothing: occurences.nothing || 0,
            blackhole: occurences.blackhole || 0,
            trader: occurences.trader || 0,
            item: occurences.item || 0,
            battle: (occurences.pirate || 0) + (occurences.alien || 0),
            duration: (occurences.early || 0) + (occurences.late || 0),
        };

        const rows = Object.entries(pieData).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]);
        const total = rows.reduce((s, [, v]) => s + v, 0) || 1;
        const max = rows.length ? rows[0][1] : 1;
        const label = k => { const l = this.ogl._lang.find(k); return l === 'TEXT_NOT_FOUND' ? (k.charAt(0).toUpperCase() + k.slice(1)) : l; };

        // ranked horizontal bars (replaces the donut): each outcome = colour dot, label, proportional
        // bar, count and %. Clearer than arcs for comparing ~8 categories (impeccable).
        Util.addDom('div', { class:'ogl_outcomeHead', parent:container, child:`<b>${Util.formatToUnits(total)}</b> expeditions` });

        const list = Util.addDom('div', { class:'ogl_outcomeList', parent:container });

        rows.forEach(([k, v]) =>
        {
            const row = Util.addDom('div', { class:'ogl_outcomeRow', parent:list });
            Util.addDom('span', { class:'ogl_outcomeDot', parent:row }).style.background = colors[k] || '#888';
            Util.addDom('span', { class:'ogl_outcomeLabel', parent:row, child:label(k) });
            const track = Util.addDom('span', { class:'ogl_outcomeTrack', parent:row });
            const fill = Util.addDom('span', { class:'ogl_outcomeFill', parent:track });
            fill.style.width = (v / max * 100).toFixed(1) + '%';
            fill.style.background = colors[k] || '#888';
            Util.addDom('span', { class:'ogl_outcomeCount', parent:row, child:Util.formatToUnits(v) });
            Util.addDom('span', { class:'ogl_outcomePct', parent:row, child:(v / total * 100).toFixed(1) + '%' });
        });
    }
    ...
    updateRecap(container, data)
    {
        ...
        rows.forEach(type =>
        {
            ...
            else if(type == 'total' || type == 'u') dataKeys = this.activeStatsTab === 'raid' ? ['raid', 'debris'] : ['expe', 'debris16', 'discovery'];
        ...
        // color negative values (losses / consumption / net deficits) red — sign clarity over resource tint
        container.querySelectorAll('.ogl_metal, .ogl_crystal, .ogl_deut, .ogl_msu, .ogl_dm, .ogl_artefact').forEach(cell =>
        {
            const t = (cell.textContent || '').trim();
            if(t.length > 1 && t[0] === '-') cell.classList.add('ogl_danger');
        });
    }
}
...
class EmpireManager extends Manager
{
    load()
    {
        ...
        // v13: the native empire page's own embedded <script> (scraped below via this.update())
        // is unstable in beta and may not contain the data at all. The official accountInfo API
        // (page=componentOnly&component=externaldataexport&action=accountInfo) covers every
        // planet/moon's resources, production, buildings, ships and defenses in one call, so use
        // it as the primary data source here - fired once (page load onto the empire page), not
        // on a timer. It does NOT expose fieldUsed/fieldMax/temperature, so those still rely on
        // whatever the current planet's own DOM/empire-script provides.
        const attachJumpgateIcons = () =>
        {
            document.querySelectorAll('.planet').forEach(planet =>
            {
                if(planet.querySelector('.ogl_empireJumpgate')) return;
        ...
        this.ogl._fetch.fetchAccountInfo(true).then(() => attachJumpgateIcons());
        // v13: field count/temperature are not in accountInfo (see FetchManager.fetchEmpireFields
        // comment) - only fetched here (landing on the empire page) and from openProduction(),
        ...
        Util.observe(document.body, {childList:true, subtree:true, attributes:true}, mutation =>
        {
            ...
            {
                // v13 compat: #loading/#mainContent may be renamed
                const _loadingEl = document.querySelector('#loading') || document.querySelector('[id*="loading"]');
                if(!_loadingEl || _loadingEl.style.display == 'none')
                {
                    updateDone = true;

                    // Fallback only: try the fragile HTML-embedded-script scrape too, in case
                    // accountInfo is unavailable on this server (pre-v13) or missing fields.
                    const _mainScript = document.querySelector('#mainContent script') || document.querySelector('[id*="mainContent"] script') || document.querySelector('[id*="main-content"] script');
                    if(_mainScript) this.update(_mainScript.innerText, new URLSearchParams(window.location.search).get('planetType'), true);

                    attachJumpgateIcons();
                }
            }
            else if(mutation.target.classList.contains('ui-dialog') && (document.querySelector('#jumpgate') || document.querySelector('[id*="jumpgate"]')))
            {
                // v13 compat: #jumpgate/#jumpgateNotReady may be renamed
                if(document.querySelector('#jumpgateNotReady') || document.querySelector('[id*="jumpgateNotReady"]')) return;
    ...
    update(data, type, fromEmpire)
    {
        ...
        // v13 compat: data may be already an object, a JSON string, or wrapped in JS var code
        if(fromEmpire)
        {
            if(typeof data === 'string')
            {
                ...
                {
                    // fallback: extract JSON object from JS assignment string
                    try
                    {
                    ...
                    {
                        console.error('EmpireManager.update: cannot parse empire data', e2);
                        data = {};
                    }
                }
            }
        ...
        const planetList = data?.planets || data?.Planets || (Array.isArray(data) ? data : null);

        planetList?.forEach(planet =>
        {
            // v13: planet.id may be planet.planetId or planet.PlanetId
            const pid = planet.id ?? planet.planetId ?? planet.PlanetId;
            if(!pid) return;

            this.ogl.db.myPlanets[pid] = this.ogl.db.myPlanets[pid] || {};
            this.ogl.db.myPlanets[pid].type = type === 0 ? 'planet' : 'moon';

            Object.entries(planet).forEach(entry =>
            {
                if(entry[0].indexOf('html') >= 0) return;
                else if(entry[0] === 'temperature') this.ogl.db.myPlanets[pid][entry[0]] = parseInt(entry[1].match(/[-|0-9]+/g, '' )[0]);
                else if(Number(entry[0]) || entry[0].indexOf('Storage') >= 0 || allowedEntries.includes(entry[0])) this.ogl.db.myPlanets[pid][entry[0].replace('deuterium', 'deut')] = parseInt(entry[1]);
                else if(entry[0] === 'coordinates' || entry[0] === 'coordinate')
                {
                    // v13: coordinates may be string '[1:1:1]' or object {galaxy,system,position}
                    const val = entry[1];
                    if(typeof val === 'string') this.ogl.db.myPlanets[pid].coords = val.replace(/[\[\]]/g, '');
                    else if(val && typeof val === 'object') this.ogl.db.myPlanets[pid].coords = `${val.galaxy}:${val.system}:${val.position}`;
                }
                else if(entry[0] === 'production')
                {
                    this.ogl.db.myPlanets[pid].prodMetal = entry[1].hourly[0] / 3600;
                    this.ogl.db.myPlanets[pid].prodCrystal = entry[1].hourly[1] / 3600;
                    this.ogl.db.myPlanets[pid].prodDeut = entry[1].hourly[2] / 3600;
                }
            });
        });
    ...
    // v13: OGame's species-bonuses export, already aggregated per entity - no DOM scraping. Ported from
    // OGLight 5.4.2. Values are stored as PERCENTAGES (x100), which is the scale every reader expects
    // (they divide by 100, or multiply by 10 for the lifeform level badges).
    updateLFBonuses(json)
    {
        if(!json?.data) return;

        const bonuses = { ...(json.data.costreduction || {}), ...(json.data.ships || {}), ...(json.data.defenses || {}) };
        ...
        for(let [key, value] of Object.entries(bonuses))
        {
            ...
            const total = value?.totalBonus || {};
            const bonusObj = {};

            if(isBaseResearch)
            {
                bonusObj.cost = (total.cost || 0) * 100;
                bonusObj.duration = (total.time || 0) * 100;
            }
            else if(isBaseShip)
            {
                bonusObj.armor = (total.armor || 0) * 100;
                bonusObj.shield = (total.shield || 0) * 100;
                bonusObj.weapon = (total.weapon || 0) * 100;
                bonusObj.speed = (total.speed || 0) * 100;
                bonusObj.cargo = (total.cargo || 0) * 100;
                bonusObj.fuel = (total.fuel || 0) * 100;
            }
            else if(isBaseDef)
            {
                bonusObj.armor = (total.armor || 0) * 100;
            ...
            // entity 120 carries the LF-research cost/time reduction read by getTechData()
            if(key == 120)
            {
                this.ogl.db.lfBonuses.LfResearch.cost = (total.cost || 0) * 100;
                this.ogl.db.lfBonuses.LfResearch.duration = (total.time || 0) * 100;
            }
        ...
        {
            this.ogl.db.lfBonuses[`lifeform${lifeform.id}`] = { bonus:(lifeform.bonus || 0) * 100, level:lifeform.level };
        });

        this.ogl.db.lfBonuses.Characterclasses1 = { bonus:(json.data.characterclasses?.['601']?.totalBonus?.characterClass || 0) * 100 };
        this.ogl.db.lfBonuses.Characterclasses2 = { bonus:(json.data.characterclasses?.['602']?.totalBonus?.characterClass || 0) * 100 };
        this.ogl.db.lfBonuses.Characterclasses3 = { bonus:(json.data.characterclasses?.['603']?.totalBonus?.characterClass || 0) * 100 };
        this.ogl.db.lfBonuses.metal = { bonus:(json.data.resources?.metal?.totalBonus?.metal || 0) * 100 };
        this.ogl.db.lfBonuses.crystal = { bonus:(json.data.resources?.crystal?.totalBonus?.crystal || 0) * 100 };
        this.ogl.db.lfBonuses.deuterium = { bonus:(json.data.resources?.deuterium?.totalBonus?.deuterium || 0) * 100 };
        this.ogl.db.lfBonuses.ResourcesExpedition = { bonus:(json.data.expedition?.ResultBooster?.totalBonus?.ResultBooster || 0) * 100 };
        this.ogl.db.lfBonuses.MiscImprovedCrawler = { bonus:(json.data.misc?.buggyBonus?.totalBonus?.resBonus || 0) * 100 };
    }
    ...
    getLFBonuses(source)
    {
        ...
        // lf bonuses - when called from live page (no source), custom elements are rendered
        // When called from DOMParser result, custom elements lack innerText
        // Detect: if we're parsing a fetched document, use textContent fallback
        const _isLivePage = !source;

        if(source || this.ogl.page == 'lfbonuses')
        {
            this.ogl.db.lfBonuses = {};

            htmlSource.querySelectorAll('inner-bonus-item-heading[data-toggable]').forEach(item =>
            {
                ...
                // a DOMParser document does not render custom elements, so a bonus row can be missing
                // entirely: reading bonuses[i][0] blindly threw and aborted the whole loop, leaving
                // db.lfBonuses half-filled (it is wiped just above). Never throw here.
                const bv = (i, j) => (bonuses[i] || [])[j] || 0;

                item.querySelectorAll('bonus-item').forEach(bonus =>
                {
                    // DOMParser doesn't render custom elements, use textContent or data attrs
                    const rawBonusText = bonus.innerText || bonus.textContent || 
                        Array.from(bonus.querySelectorAll('[data-value],[value]'))
                            .map(e => e.getAttribute('data-value') || e.getAttribute('value')).join(' ');
                    const values = (rawBonusText.match(regex) || []).map(e =>
                    {
                        if(e == '-') return 0;
                ...
                if(bonuses.length == 0)
                {
                    // innerText is empty on custom elements parsed via DOMParser (no JS execution)
                    // Try multiple sources: innerText, textContent, data-value, or any span/div with a number
                    let rawText = item.innerText || item.textContent || '';
                    // Also check for nested text in spans/divs (OGame renders bonuses in child elements)
                    if(!rawText.trim()) {
                        const allText = Array.from(item.querySelectorAll('*'))
                            .map(e => e.getAttribute('data-value') || e.getAttribute('value') || '')
                            .filter(Boolean).join(' ');
                        rawText = allText || rawText;
                    }
                    let value = (rawText.match(regex) || [])[0];
                ...
                if(isBaseShip)
                {
                    lfBonuses.armor = bv(0, 0);
                    lfBonuses.shield = bv(1, 0);
                    lfBonuses.weapon = bv(2, 0);
                    lfBonuses.speed = bv(3, 0);
                    lfBonuses.cargo = bv(4, 0);
                    lfBonuses.fuel = bv(5, 0);
                }
                else if(isBaseDef)
                {
                    lfBonuses.armor = bv(0, 0);
                ...
                {
                    lfBonuses.cost = bv(0, 0);
                    lfBonuses.duration = bv(1, 0);
                }

                if(id == 'MiscImprovedCrawler')
                {
                    lfBonuses.bonus = bv(1, 0);
                }
        ...
        // New OGame: read totals from inner-bonus-item-heading .subCategoryBonus
        // This replaces the old Characterclasses3 / ResourcesExpedition keys
        if(source || this.ogl.page == 'lfbonuses')
        {
            this.ogl.db.lfBonuses = this.ogl.db.lfBonuses || {};
            htmlSource.querySelectorAll('inner-bonus-item-heading').forEach(el =>
            {
                const key = el.getAttribute('data-toggable');
                if(!key) return;
                const bonusText = el.querySelector('.subCategoryBonus')?.textContent || '';
                // Handle "105,01%" and "Totale: 32,81%" formats
                const bonusVal = parseFloat(bonusText.replace(/[^0-9,.]/g, '').replace(LocalizationStrings.decimalPoint, '.'));
                if(isNaN(bonusVal) || bonusVal === 0) return;

                // MERGE, never replace: the block above already stored { cost, duration } under this same
                // key (the v13 data-toggable values are plain, so its stripped `id` === this `key`).
                // Replacing the entry wiped `.duration`, so the research / LF-research duration factor
                // (1 - bonus.duration) collapsed to 1 and every computed duration came out 1/(1-x) times
                // too long (e.g. a 70% research-time reduction made durations 3.3x too high).
                this.ogl.db.lfBonuses[key] = Object.assign(this.ogl.db.lfBonuses[key] || {}, { bonus: bonusVal });

                // Map new keys to old keys used in calcExpeditionMax (same merge rule, same reason)
                if(key === 'ResultBooster') this.ogl.db.lfBonuses['ResourcesExpedition'] = Object.assign(this.ogl.db.lfBonuses['ResourcesExpedition'] || {}, { bonus: bonusVal });
                if(key === '603') this.ogl.db.lfBonuses['Characterclasses3'] = Object.assign(this.ogl.db.lfBonuses['Characterclasses3'] || {}, { bonus: bonusVal });
            });
        }
    }
...
class Util
{
    ...
    static drawLine(svg, parent, element1, element2)
    {
        ...
        // v13 compat: .planetPic/.icon-moon may be renamed
        element1 = element1.querySelector('.planetPic, .icon-moon, img') || element1;
        element2 = element2.querySelector('.planetPic, .icon-moon, img') || element2;
    ...
    static genTrashsimLink(ogl, apiKey, data)
    {
        ...
        // osims uses the spy-report key (SR_KEY) and no lang path
        if(toolID == 'osims')
        {
            if(apiKey) return link + '?SR_KEY=' + apiKey + '#prefill=' + jsonData;
            else return link + '#prefill=' + jsonData;
        }
    ...
    static genConverterLink(ogl, apiKey)
    {
        ...
        if(toolID == 'ogotcha') return `${link}${lang}?CR_KEY=${apiKey}&utm_source=PrOGect`;
    ...
    {
        let lang = ogl.account.lang == 'us' ? 'en' : ogl.account.lang == 'ar' ? 'es' : ogl.account.lang;
        return `https://ogotcha.universeview.be/${lang}?CR_KEY=${apiKey}&utm_source=PrOGect`;
    }
    ...
    // PrOGect: player stats moved from Mmorpg-stat to OStats (https://ostats.eu/)
    static genOstatsLink(ogl, playerID)
    {
        return `https://ostats.eu/universes/s${ogl.server.id}-${ogl.server.lang}/player/${playerID}`;
    }
}
...
class PTRE
{
    ...
    // PrOGect: builds the PTRE errors list container (hosted inline by settings via openDetail).
    buildLogs()
    {
    ...
    // standalone popup (from the left-menu PTRE status icon)
    displayLogs()
    {
        const container = this.buildLogs();
